...
## Recent: Traditional SEO keyword demand (2026-03-13)

- ✅ **Supabase:** `keyword_target_metrics_cache` — keyed by normalised `(page_url, keyword)`; stores `search_volume`, `cpc`, `competition`, optional `rank_position`, `moz_domain_authority`, `fetched_at`, `raw_payload`.
- ✅ **API:** `POST /api/aigeo/keyword-target-metrics` — `action: "lookup"` (read cache only, no external API), `action: "refresh"` (Keywords Everywhere → upsert stale/missing rows). Env: `KEYWORDS_EVERYWHERE_API_KEY`, optional `KEYWORDS_EVERYWHERE_COUNTRY` / `KEYWORDS_EVERYWHERE_CURRENCY`, `KEYWORD_METRICS_STALE_DAYS` (default 30).
- ✅ **UI (`audit-dashboard.html`):** Results table columns **Kw vol**, **Rank**, **Moz DA**, **Metrics age**. **③ Refresh keyword demand (KE)** next to ①/② and duplicated next to **Rows per page**. **①/② do not call KE** — only ③ (or automatic `lookup` after a Traditional SEO run).
...
|--------|---------|-----------|-------|
| **`Docs/`** | All markdown documentation | ✅ Yes | All MD files except README.md, HANDOVER.md, and AGENTS.md |
...
- `AGENTS.md` - Short index for AI coding agents (pointers + gotchas)
...
### Module: Traditional SEO
| Button | Element ID / selector | Function | Data Fetched |
|--------|------------------------|----------|--------------|
| ② Score Traditional SEO only | `traditional-seo-run-audit-btn` | `runTraditionalSeoEvaluation({ refreshAudit: false })` | Saved audit + extractability refresh + rule matrix |
| ① Full audit refresh + Traditional SEO | `traditional-seo-run-full-audit-btn` | `runTraditionalSeoEvaluation({ refreshAudit: true })` | Full `runAudit()` then Traditional SEO |
| ③ Refresh keyword demand (KE) | `[data-traditional-seo-refresh-kw="1"]` (two buttons) | `traditionalSeoRefreshKeywordMetricsFromProvider()` | **Keywords Everywhere** → Supabase `keyword_target_metrics_cache` (missing/stale only) |

**Note:** After ①/②, the client runs **`lookup`** only to populate **Kw vol / Metrics age** from cache. It never calls KE on passive load.
...
**`keyword_target_metrics_cache`** (2026-03):
- Cached **keyword demand** (and optional rank/Moz) per normalised `page_url` + `keyword`
- Written by `/api/aigeo/keyword-target-metrics` `refresh`; read by `lookup`
- Migration: `sql/20260321_keyword_target_metrics_cache.sql`
...
10. `AGENTS.md` - Short index for AI coding agents (start here + pointers)
11. `Docs/TRADITIONAL_SEO_KEYWORD_METRICS.md` - Keywords Everywhere cache + Traditional SEO columns + API/env
12. `Docs/CHANGELOG.md` - Version history
13. `Docs/AI_HEALTH_SCORE_INCONSISTENCY.md` - Authority/Visibility mismatch diagnosis
...
### What's Working ✅
- Traditional SEO results table: **Kw vol** + **Metrics age** from Supabase cache; **③** triggers Keywords Everywhere refresh for missing/stale pairs (`Docs/TRADITIONAL_SEO_KEYWORD_METRICS.md`)
