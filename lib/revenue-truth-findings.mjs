...
function buildUnitFindings(buckets, unitType, ctx) {
  return buckets.map(b => buildOneFinding(b, unitType, ctx));
}
...
function detectFlags(b, series, meta) {
  ...
  if (isNewLaunch(series)) flags.push('new_launch');
...
function flagSentenceText(flags, oneOff) {
  ...
  if (flags.includes('new_launch'))        out.push('This is a new product/page introduced after 2024.');
...
function collectFlags(productFindings, pageFindings) {
  ...
  const launches  = collectByFlag([...productFindings, ...pageFindings], f => f.flags.includes('new_launch'), f => ({ unit_type: f.unit_type, unit_id: f.unit_id, note: `${f.unit_id} first earned non-JLR revenue in 2025 or 2026 - any year-on-year 'change' is a launch, not a decline.` }));
  return { one_offs: oneOffs, retired_wind_downs: retired, new_launches: launches };
}
