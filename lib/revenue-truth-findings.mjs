...
const TIMING_FUNDING = new Set(['picknmix', 'gift voucher out']);

function isTimingFunding(funding) {
  return TIMING_FUNDING.has(String(funding || '').trim().toLowerCase());
}

function normaliseTxn(t) {
  const src = String(t.booking_source || '').trim();
  const txnDate = t.txn_date ? String(t.txn_date).slice(0, 10) : null;
  return {
    year: Number(t.year),
...
function aggregateByUnit(ctx, unitType) {
  ...
  for (const t of ctx.txns) {
    // Pick'n'Mix / Gift Voucher Out are paired +/- timing lines — net to £0 in
    // §8 funding but inflate product/page sums when only the gross leg carries
    // a product or page assignment. Exclude from unit breakdowns; see category
    // grid ADJUSTMENT column for those movements.
    if (isTimingFunding(t.funding)) continue;
...
function buildOneFinding(b, unitType, ctx) {
  ...
  return {
    ...
    last_txn_date_nonjlr: lastTxnDate(b, true),
    last_txn_date_total: lastTxnDate(b, false),
...
function lastTxnDate(b, nonjlrOnly) {
  let max = null;
  for (const t of b.txns) {
    if (nonjlrOnly && t.is_jlr) continue;
    const d = t.txn_date;
    if (!d) continue;
    if (!max || d > max) max = d;
  }
...
function trendSentence(subject, s) {
  ...
  const ytd = s.nonjlr.y2026_ytd_closed ?? s.nonjlr.y2026_ytd;
  if (a === 0 && c > 0) {
...
/** Extend closed-month seasonal forecast with current-month blended non-JLR projection. */
export function buildLiveSeasonalForecast(seasonalForecast, currentMonthProjectedNonJlr) {
  if (!seasonalForecast) return null;
  const closedM = seasonalForecast.closed_months_current_year || 0;
  const proj = Number(currentMonthProjectedNonJlr);
  if (!Number.isFinite(proj) || closedM <= 0 || closedM >= 12) {
    return {
      total_full_year_mid: seasonalForecast.total_full_year_mid,
      total_full_year_low: seasonalForecast.total_full_year_low,
      total_full_year_high: seasonalForecast.total_full_year_high,
      method_label: seasonalForecast.method_label,
      basis: 'closed_months_only'
    };
  }
  const perCat = seasonalForecast.forecast_per_category || [];
  const ytdClosed = perCat.reduce((s, p) => s + (Number(p.ytd_closed_nonjlr) || 0), 0);
  const extendedM = closedM + 1;
  const wSum = weightedWeightSumThroughMonth(perCat, extendedM);
  if (wSum < FORECAST_CLOSED_WEIGHT_MIN) return null;
  const ytdIncl = round2(ytdClosed + proj);
  const mid = round2(ytdIncl / wSum);
  return {
    total_full_year_mid: mid,
    total_full_year_low: round2(mid * (1 - FORECAST_RANGE_PCT)),
    total_full_year_high: round2(mid * (1 + FORECAST_RANGE_PCT)),
    method_label: 'Seasonally adjusted — YTD closed + current-month blended (non-JLR)',
    basis: 'incl_current_month',
    ytd_incl_current_month: ytdIncl,
    closed_weight_sum: round3(wSum),
    closed_months_included: extendedM
  };
}

function weightedWeightSumThroughMonth(perCat, monthM) {
  let num = 0;
  let den = 0;
  for (const p of perCat) {
    const w = p.monthly_weights;
    if (!w) continue;
    const ytd = Math.abs(Number(p.ytd_closed_nonjlr) || 0);
    if (ytd <= 0) continue;
    let s = 0;
    for (let i = 0; i < monthM && i < 12; i++) s += Number(w[i]) || 0;
    num += s * ytd;
    den += ytd;
  }
