...
export function recomputeAuthorityTotal(behaviour, ranking, backlinks, reviews) {
  ...
  return clampAuthorityScore(
    AUTHORITY_WEIGHTS.behaviour * behaviour +
      AUTHORITY_WEIGHTS.ranking * ranking +
      AUTHORITY_WEIGHTS.backlinks * backlinks +
      AUTHORITY_WEIGHTS.reviews * reviews
  );
}
...
export function resolveAuthorityFromHistoryRecord(record, lastGood = {}) {
  ...
  const total = recomputeAuthorityTotal(filled[0], filled[1], filled[2], filled[3]);
  if (total === null) {
    if (typeof stored === 'number' && Number.isFinite(stored)) {
