...
import { recomputeAuthorityTotal } from './authorityScore.js';
import {
  RANKING_CRITERIA,
  isTop10Position,
  prepareRankingPool
...
const computeBehaviourScoreRaw = (queries) => {
  ...
  const ranking = prepareRankingPool(queries);
  ...
  const top10 = ranking.filter((q) => isTop10Position(q.position));
...
const computeRankingScoreRaw = (queries) => {
  ...
  const clampedPos = Math.max(1, Math.min(RANKING_CRITERIA.POOL_MAX_POSITION, avgPos));
  const posScore = normalisePosition(clampedPos, 1, RANKING_CRITERIA.POOL_MAX_POSITION);

  const top10Impr = ranking.filter((q) => isTop10Position(q.position)).reduce((s, q) => s + q.impressions, 0);
  const top10Share = totalImpr > 0 ? top10Impr / totalImpr : 0;
  const top10Score = top10Share * 100;

  return RANKING_CRITERIA.POSITION_BLEND * posScore + RANKING_CRITERIA.TOP10_BLEND * top10Score;
};
...
const computeBacklinkScore = (m) => {
  if (!m) return 0;
  const rd = Number(m.referringDomains ?? 0);
  const fb = Number(m.followRatio ?? 0);

  if (rd === 0 && (m.totalBacklinks ?? 0) === 0) return 0;

  const rdScore = Math.min(rd / 100, 1) * 100;
  const followScore = Math.max(0, Math.min(fb, 1)) * 100;
  return clampScore(0.7 * rdScore + 0.3 * followScore);
};
...
const calculateSnippetReadiness = (scores) => {
  ...
  const visibilityScore = (typeof scores.visibility === 'object' && scores.visibility)
    ? (scores.visibility.score ?? scores.visibility.overall ?? 0)
    : (scores.visibility || 0);
  const readiness = (
    (scores.contentSchema || 0) * 0.4 +
    visibilityScore * 0.35 +
    (authorityScore || 0) * 0.25
  );
  return Math.round(Math.min(100, Math.max(0, readiness)));
};

/** Signed 2026-07-16: 35% AI-citation + 25% surface Visibility + 25% snippet + 15% Brand demand */
const computeAiSummaryLikelihood = (snippetReadinessScore, visibilityScore, brandScore, aiCitationRate = 0) => {
  const snip = Number(snippetReadinessScore) || 0;
  const vis = Number(visibilityScore) || 0;
  const brand = Number(brandScore) || 0;
  const cite = Number(aiCitationRate) || 0;
  const composite = 0.35 * cite + 0.25 * vis + 0.25 * snip + 0.15 * brand;
  let label = 'High';
...
const computeSegmentedScores = (queryPages, topQueries) => {
  if (Array.isArray(queryPages) && queryPages.length > 0) {
    ...
    return {
      behaviourScore: behaviourScoresSegmented.money,
      rankingScore: rankingScoresSegmented.money,
...
const computeAuthorityScores = ({
  ...
  const computeAuthorityFromComponents = (components) => {
    const total = recomputeAuthorityTotal(
...
const computeRichResultScore = (schemaData) => {
  const richResultTypes = ['Article', 'Event', 'FAQPage', 'Product', 'LocalBusiness', 'Review', 'HowTo', 'VideoObject', 'ImageObject', 'ItemList'];
...
/** Audit-time fallback (GSC-only). Live UI prefers /api/aigeo/brand-demand (GBP + GSC tables). */
const computeBrandOverlayScore = (queryPages, topQueries) => {
  ...
  return computeBrandOverlay({
    gbpInteractions: 0,
    gbpProfileImpressions: 0,
    brandClicks: brandMetrics.brandClicks || 0,
...
const calculatePillarScores = (data, schemaAudit = null, localSignals = null, siteReviews = null, backlinkMetrics = null) => {
  ...
  const brandOverlay = computeBrandOverlayScore(queryPages, topQueries);
