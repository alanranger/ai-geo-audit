...
const buildRollingDateRange = (daysBack = 28, endOffsetDays = 2) => {
  const end = new Date();
...
const runFullAudit = async ({
  ...
  const { startDate, endDate } = buildRollingDateRange(dateRangeDays, 2);
  ...
  let backlinkMetrics = backlinkMetricsResponse?.data || backlinkMetricsResponse || null;

  const pageLevel = await fetchJson(`${baseUrl}/api/aigeo/gsc-page-level`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ propertyUrl, startDate, endDate })
  });

  if (!backlinkMetrics) {
    try {
      const latestAuditResp = await fetchJson(
        `${baseUrl}/api/supabase/get-latest-audit?propertyUrl=${encodeURIComponent(propertyUrl)}&includePartial=true&preferRecent=true`
      );
      backlinkMetrics = latestAuditResp?.data?.backlinkMetrics || backlinkMetrics;
    } catch {
  ...
  const pageTitleLookup = (() => {
    const pages = schemaAudit?.data?.pages;
  ...
  let moneyPagesTimeseries = [];

  try {
    if (moneyPagesList.length > 0) {
      const pageTimeseriesResp = await fetchJson(`${baseUrl}/api/aigeo/gsc-page-timeseries`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          propertyUrl,
          startDate,
          endDate,
          pages: moneyPagesList.map((row) => row.url).filter(Boolean)
        })
      });
      moneyPagesTimeseries = pageTimeseriesResp?.data || [];
    }
  } catch (err) {
    // Non-blocking: timeseries is optional for page-level charts
    console.warn('[Full Audit] Page timeseries fetch failed:', err?.message || err);
    moneyPagesTimeseries = [];
  }

  return {
    gscRange: { startDate, endDate },
    gscPageRows: pageRows,
    searchData,
    schemaAudit,
    localSignals,
    siteReviews,
