 ...
 *
 * Per-keyword weighted average of organic / pack / aio / fs_paa / paa / kp,
...
export const CLASS_WEIGHTS = Object.freeze({
  // organic-funded PAA (Alan 2026-08-10): local organics 30→25; national organic 45→40 + aio 30→27.
  'local-money': Object.freeze({ pack: 35, organic: 25, aio: 25, fs_paa: 10, paa: 5, kp: 0 }),
  // Retired 2026-08: regional tier emptied + remapped. Keep weights only for stale rows.
  'regional-money': Object.freeze({ pack: 25, organic: 38, aio: 28, fs_paa: 12, paa: 0, kp: 0 }),
  'national-money': Object.freeze({ pack: 10, organic: 40, aio: 27, fs_paa: 15, paa: 8, kp: 0 }),
  brand: Object.freeze({ pack: 15, organic: 35, aio: 10, fs_paa: 0, paa: 0, kp: 40 }),
  education: Object.freeze({ pack: 0, organic: 40, aio: 35, fs_paa: 25, paa: 0, kp: 0 }),
});

/** Classes that contribute to the headline Surface Visibility overall dial. */
export const SURFACE_OVERALL_MONEY_CLASSES = Object.freeze(['local-money', 'national-money']);

const SURFACES = ['pack', 'organic', 'aio', 'fs_paa', 'paa', 'kp'];
...
/**
 * Featured snippet ownership (fs_paa surface = FS only — MC-57).
 * PAA is a separate scored surface via scorePaa.
 */
export function scoreFsPaa(fsOurs, paaOurs) {
  if (fsOurs) return 100;
  if (paaOurs) return 60; // legacy path; callers pass false for PAA
  return 0;
}

/** People Also Ask ownership. Only when PAA served (stack-gated). Binary win/lose. */
export function scorePaa(paaOurs) {
  return paaOurs ? 100 : 0;
}
...
export function computeKeywordSurfaceScore(row) {
  ...
  // MC-57 Option A: fs_paa surface = featured snippet only, stack-gated.
  const fsStack = stack.filter((e) => e.type === 'featured_snippet' && e.slot != null);
  const fsPresent = fsStack.length > 0;
  const fsOurs = fsStack.some((e) => e.ours === true);
  // PAA scored separately — stack-gated only (never paa_present_any / paa_ours flags alone).
  const paaStack = stack.filter((e) => e.type === 'people_also_ask' && e.slot != null);
  const paaPresent = paaStack.length > 0;
  const paaOurs = paaStack.some((e) => e.ours === true);
  ...
  const subscores = {
    ...
    fs_paa: fsPresent ? scoreFsPaa(fsOurs, false) : null,
    paa: paaPresent ? scorePaa(paaOurs) : null,
...
export function computeSurfaceVisibilityRollup(rows) {
  ...
  // Headline overall = local + national money only (regional retired; brand/education remain byClass-only).
  const overallRows = perKeyword.filter((e) => SURFACE_OVERALL_MONEY_CLASSES.includes(e.surface.keyword_class));
  const overall = demandWeightedMean(overallRows);
  let weakestClass = null;
  let weakestScore = Infinity;
  for (const cls of SURFACE_OVERALL_MONEY_CLASSES) {
    const d = dials[cls];
