...
const buildMoneyRow = ({
  ...
  const classified = classifyMoneyPage(url);
  if (!classified.isMoney) return null;
  ...
  const moneyRole = classified.moneyRole;
  const inHeadline = !!classified.includeInHeadline;

  const bucket = summaryByCategory[category];
  if (bucket && inHeadline) {
    bucket.count += 1;
...
const collectMoneyRows = ({
  ...
  topPages.forEach((page) => {
    ...
    // Headline KPIs (click share etc.) = commercial landing set only
    if (row.includeInHeadline && row.impressions > 0) {
...
const classifyMoneyPage = (url) => {
  if (!url) return { isMoney: false, segmentType: null, moneyRole: null };
  if (isUtilityMoneyPath(url)) {
    return { isMoney: false, segmentType: null, moneyRole: 'utility' };
  }
  const segment = classifyPageSegment(url);
  if (segment !== PageSegment.MONEY) {
    return { isMoney: false, segmentType: null, moneyRole: null };
  }
  const subSegment = classifyMoneyPageSubSegment(url);
  let segmentType = 'all';
  ...
  const moneyRole = moneyRoleForUrl(url, segmentType);
  return {
    isMoney: showInMoneyTab(moneyRole),
    segmentType,
    moneyRole,
    includeInHeadline: includeInMoneyHeadline(moneyRole)
  };
};
...
const buildMoneyPageMetrics = (topPages, schemaAudit = null) => {
  ...
  for (const row of topPages) {
    ...
    const { isMoney, segmentType, moneyRole, includeInHeadline } = classifyMoneyPage(url);
    if (!isMoney) continue;
    ...
    const m = {
      ...
      moneyRole: moneyRole || null,
      includeInHeadline: !!includeInHeadline,
      clicks: row.clicks || 0,
  ...
  // Pinned impact bands: commercial set + funnel pages (funnel excluded from headline scale)
  const banded = result.filter(
    (p) => (p.includeInHeadline && !excludeFromImpactScale(p.moneyRole)) || p.moneyRole === 'funnel'
  );
  computeImpactLevels(banded);
  for (const p of result) {
    if (!p.includeInHeadline && p.moneyRole !== 'funnel') {
      p.impactLevel = 'LOW';
      p.priorityLevel = 'LOW';
      continue;
    }
...
const buildMoneySegmentSummary = (moneyPages, behaviourScores = {}) => {
  ...
  const groups = {
    // Headline "all money" = commercial landing hubs only (v2)
    allMoney: pages.filter((p) => p.includeInHeadline === true || p.moneyRole === 'commercial'),
    landingPages: pages.filter((p) => p.segmentType === 'landing' && p.moneyRole !== 'utility'),
