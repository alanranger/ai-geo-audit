...
import { scoreOrganic, SURFACE_OVERALL_MONEY_CLASSES } from './surfaceScores.js';

export const TOP_OF_PAGE_SCHEMA_VERSION = 3;

/** Rebased 2026-07-16 (hyperlocal Local-tier era). */
export const TOP_OF_PAGE_BASELINE_DATE = '2026-07-16';
...
export function packMultiplier(position) {
  ...
  if (p <= 0) return 0;
  if (p <= 1) return 1.0;
  if (p <= 2) return 1.0 - (p - 1) * 0.15; // 1→1.0, 2→0.85
  if (p <= 3) return 0.85 - (p - 2) * 0.15; // 2→0.85, 3→0.70
  return 0.5;
}
...
export function computeKeywordTopOfPageScore(row) {
  ...
  // Breadth = additional owned SURFACE TYPES (not appearances). KP is rail-side (slot null) and excluded.
  const ownedSurfaceTypes = [...new Set(owned.map((el) => el.type))];
  const extraOwnedTypes = Math.max(0, ownedSurfaceTypes.length - (bestScore > 0 ? 1 : 0));
  const breadthBonus = Math.min(10, extraOwnedTypes * 5);
  ...
  return {
    ...
    components: {
      ...
      owned_surface_types: ownedSurfaceTypes,
...
export function computeTopOfPageRollup(rows) {
  ...
  return {
    ...
    overall: demandWeightedMean(
      perKeyword.filter((e) => SURFACE_OVERALL_MONEY_CLASSES.includes(e.top.keyword_class)),
    ),
    byClass: dials,
