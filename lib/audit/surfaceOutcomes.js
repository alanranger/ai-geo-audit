...
const SURFACE_DEFS = [
  { key: 'ai_answer', label: 'Google AI Overview', served: aioServed, owned: aioOwned },
  {
    key: 'google_ai_mode',
...
export function computeSurfaceOutcomesRollup(rows) {
  ...
  let biggest = null;
  // Callout: AI Overview / Map / Organic only — skip Answer boxes + diagnostic AI Mode.
  for (const row of tableRows) {
    if (row.key === 'answer_boxes' || row.diagnostic || row.key === 'google_ai_mode') continue;
  ...
  if (biggest && biggest.gapVol > 0 && moneyVol > 0) {
    ...
    callout = {
      ...
      text: biggest.key === 'ai_answer'
        ? `${biggest.gapPctOfMoney}% of money-search demand is in Google AI Overviews you're not cited in${suffix}.`
