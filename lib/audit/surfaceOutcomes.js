...
export function computeSurfaceOutcomesRollup(rows) {
  ...
  if (biggest && biggest.gapVol > 0 && moneyVol > 0) {
    ...
    callout = {
      ...
      text: `${biggest.gapPctOfMoney}% of money-search demand is in ${biggest.label.toLowerCase()}s you're not cited in${suffix}.`,
    };
    // AI answer wording matches signed brief ("AI answers")
    if (biggest.key === 'ai_answer') {
      callout.text = `${biggest.gapPctOfMoney}% of money-search demand is in AI answers you're not cited in${suffix}`;
    }
  }
