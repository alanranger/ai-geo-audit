...
export function computeSurfaceOutcomesRollup(rows) {
  ...
  if (biggest && biggest.gapVol > 0 && moneyVol > 0) {
    ...
    let suffix = '';
    if (classGaps.length === 1) suffix = ` — all ${classGaps[0].label}`;
    else if (classGaps.length > 1) {
      const topShare = classGaps[0].gap / gapTotal;
      if (topShare >= 0.7) suffix = ` — mostly ${classGaps[0].label}`;
      else suffix = ` — mostly ${classGaps.slice(0, 2).map((c) => c.label).join(' + ')}`;
    }
    callout = {
