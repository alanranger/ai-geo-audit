...
export const HUB_INVESTIGATE_SLUGS = new Set([
  'photography-courses-coventry',
  'hire-a-professional-photographer-in-coventry'
]);

export function isHubInvestigateSlug(pageSlug) {
  const s = String(pageSlug || '').replace(/^\/+|\/+$/g, '');
  return HUB_INVESTIGATE_SLUGS.has(s);
}
...
export function auditExecSummaryBullets(summary, windowMonths) {
  const issues = [];
  const worryItems = summary?.bullets?.worry || summary?.cards?.worry?.items || [];
  if (worryItems.length > WORRY_MAX_BULLETS) {
    issues.push(`Worry card has ${worryItems.length} items (max ${WORRY_MAX_BULLETS})`);
  }
  const blocked = [
    'about-alan-ranger',
    'testimonials',
  ...
  for (const item of worryItems) {
    const text = String(item.text || item.label || item.detail || '').toLowerCase();
    for (const token of blocked) {
      if (text.includes(token)) issues.push(`Worry mentions blocked token "${token}": ${item.text || item.label}`);
    }
  }
  return { ok: issues.length === 0, issues, windowMonths, worryCount: worryItems.length };
}
