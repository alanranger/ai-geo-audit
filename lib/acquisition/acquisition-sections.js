 ...
 */
import { ga4AttributedView, engagementRag } from './ga4-channels.js';
...
const tile = (label, value, opts = {}) => ({
  ...
  sub: opts.sub ?? null,
  text: opts.text ?? null,
  engaged_pct: opts.engaged_pct ?? null,
  avg_session_seconds: opts.avg_session_seconds ?? null,
  engaged_band: opts.engaged_band ?? null,
  duration_band: opts.duration_band ?? null,
...
/**
 * Headline total — attributed GA4 sessions with bots held aside.
 *
 * Sits above the per-channel breakdown so the tab leads with one comparable
 * number (with period deltas) rather than asking the reader to infer it from
 * six channel tiles plus a reconcile footnote.
 */
export function ga4RealVisitsSection(ga4Rows) {
  const base = {
    key: 'ga4_real_visits',
    source: 'GA4',
    title: 'Real site visits',
    unit: 'visits',
    subtitle: 'Attributed GA4 sessions — Unassigned bot traffic excluded',
    tiles: [],
    checks: [],
    total: null,
  ...
  const rawExcluded = view.unattributed_sessions || 0;
  const excludedPct = view.attributed_pct == null ? null : 100 - view.attributed_pct;
  const botNote = rawExcluded
    ? `${rawExcluded.toLocaleString()} Unassigned sessions excluded${excludedPct == null ? '' : ` (${excludedPct}% of raw GA4)`}`
    : 'No Unassigned bot traffic in this period';
  return {
    ...base,
    tiles: [
      tile('Total visits', view.total_sessions, {
        unit: 'visits',
        sub: rawExcluded ? `excl. ${rawExcluded.toLocaleString()} bots` : 'visits',
        ...ga4EngageFields(view),
        accent: SOURCES.GA4.colour,
      }),
    ],
    total: view.total_sessions,
    unattributed_sessions: view.unattributed_sessions,
    attributed_pct: view.attributed_pct,
    explainer: `GA4 attributed sessions on alanranger.com. ${botNote}. Internal traffic is filtered in GA4 before data reaches this tab.`,
  };
}

function ga4EngageFields(metrics) {
  const { engaged_pct, avg_session_seconds } = metrics;
  return { engaged_pct, avg_session_seconds, ...engagementRag({ engaged_pct, avg_session_seconds }) };
}

function ga4Tiles(view) {
  const engage = (g) => ga4EngageFields(g);
  return view.tiles.map((g, i) => {
    ...
    if (g.name === 'Direct') {
      return tile(label, g.sessions, { unit: 'visits', sub: 'visits · untagged', accent, ...engage(g) });
    }
    if (g.name === 'Other') {
      return tile('Other', g.sessions, {
        unit: 'visits',
        sub: `visits · ${g.groups.join(', ')}`,
        accent,
        ...engage(g),
      });
    }
    return tile(label, g.sessions, { unit: 'visits', sub: 'visits', accent, ...engage(g) });
  });
}
