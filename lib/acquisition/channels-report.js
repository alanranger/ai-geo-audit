...
export function youtubeWindowViews(snapshots) {
  ...
  if (rows.length === 0) {
    return { value: null, note: 'awaiting YouTube authorisation — no snapshot has been pulled yet' };
  }
  const analytics = rows.find((r) => r.views_window != null);
  if (analytics) return { value: Number(analytics.views_window), note: null };
  if (rows.length < 2) {
    return {
      value: null,
      note: 'channel connected, but period views need either the yt-analytics.readonly scope or a second daily snapshot (history starts 29 Aug 2026)',
    };
  }
  ...
  return { value: Math.max(0, delta), note: 'derived from daily snapshot delta' };
}

const NOT_MEASURABLE = 'no referrer source wired — visits from this channel are not measurable yet';

function aiChannel(base, monthly, latestDaily) {
  ...
  return {
    ...
    visits: { value: null, note: NOT_MEASURABLE },
    state: 'ok',
  };
}
...
export function buildChannelRows(sources) {
  const { gscTotals, llmMonthly, llmLatest, youtubeSnapshots, academyByChannel } = sources;
  ...
  const organic = {
    ...
    cited: null,
    visits: { value: gscTotals?.clicks ?? null, note: null },
    state: 'ok',
  };

  const chatgpt = aiChannel(find('chatgpt'), llmMonthly?.chat_gpt, llmLatest?.chat_gpt);
  const googleAi = aiChannel(find('google_ai'), llmMonthly?.google, llmLatest?.google);
  ...
  const youtube = {
    ...
    cited: null,
    visits: { value: null, note: NOT_MEASURABLE },
    state: ytViews.value == null ? 'awaiting_setup' : 'ok',
  };
