...
const emptyEngagement = () => ({ sessions: 0, engaged: 0, durationSum: 0 });

function addEngagement(acc, row) {
  const sessions = Number(row?.sessions || 0);
  const engaged = Number(row?.engaged_sessions || 0);
  const avgSec = Number(row?.avg_session_seconds);
  acc.sessions += sessions;
  acc.engaged += engaged;
  if (sessions > 0 && Number.isFinite(avgSec)) acc.durationSum += avgSec * sessions;
  return acc;
}

/** Session-weighted engaged rate and average duration for one channel bucket. */
export function engagementFromAcc(acc) {
  if (!acc?.sessions) return { engaged_pct: null, avg_session_seconds: null };
  return {
    engaged_pct: Math.round((acc.engaged / acc.sessions) * 1000) / 10,
    avg_session_seconds: Math.round(acc.durationSum / acc.sessions),
  };
}

function groupEngagement(rows) {
  const stats = new Map();
  let overall = emptyEngagement();
  for (const r of rows || []) {
    if (r?.is_unattributed) continue;
    const group = r?.channel_group || '(unknown)';
    const cur = stats.get(group) || emptyEngagement();
    stats.set(group, addEngagement(cur, r));
    overall = addEngagement(overall, r);
  }
  return { stats, overall: engagementFromAcc(overall) };
}

/** Human-readable session length for Acquisition tiles. */
export function formatGa4Duration(seconds) {
...
/**
 * RAG bands for GA4 engagement quality on attributed traffic.
 *
 * Calibrated to this property: Unassigned bots sit ~3% engaged / ~5s; real
 * channels cluster ~48–52% engaged / 69–172s. Thresholds flag bot-like vs human.
 */
export const ENGAGED_PCT_GREEN = 45;
export const ENGAGED_PCT_AMBER = 25;
export const DURATION_SEC_GREEN = 90;
export const DURATION_SEC_AMBER = 30;

export const ENGAGEMENT_RAG_NOTE =
  'Engaged rate: green ≥45%, amber 25–44%, red <25%. '
  + 'Avg session: green ≥90s, amber 30–89s, red <30s. '
  + 'Based on GA4 engaged-session definition; bots on this property sit ~3% / ~5s.';

export function engageRagBand(value, greenMin, amberMin) {
...
export function engagementRag({ engaged_pct, avg_session_seconds }) {
  return {
    engaged_band: engageRagBand(engaged_pct, ENGAGED_PCT_GREEN, ENGAGED_PCT_AMBER),
    duration_band: engageRagBand(avg_session_seconds, DURATION_SEC_GREEN, DURATION_SEC_AMBER),
  };
}

/** Gate period deltas until the prior window has enough GA4 channel history. */
export function ga4PriorComparable(priorDays, periodDays) {
  const need = Math.min(periodDays, Math.max(14, Math.floor(periodDays * 0.5)));
  return priorDays >= need;
}
...
export function ga4AttributedView(rows) {
  ...
  const { stats, overall } = groupEngagement(rows);
  const total = [...totals.values()].reduce((t, n) => t + n, 0);

  const tileEngagement = (groups) => {
    const acc = emptyEngagement();
    ...
    return engagementFromAcc(acc);
  };

  const tiles = [];
  for (const name of NAMED_GA4_GROUPS) {
    if (totals.has(name)) {
      tiles.push({ name, sessions: totals.get(name), groups: [name], ...tileEngagement([name]) });
    }
  }
  ...
  return {
    ...
    engaged_pct: overall.engaged_pct,
    avg_session_seconds: overall.avg_session_seconds,
    ...validation,
  };
}
