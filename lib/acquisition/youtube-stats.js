...
export function missingSetup(cfg = youtubeConfig()) {
  const missing = [];
  if (cfg.oauth?.ready) return missing;
  if (!cfg.apiKey) missing.push('YOUTUBE_API_KEY (or owner OAuth: YOUTUBE_CLIENT_ID + YOUTUBE_CLIENT_SECRET + YOUTUBE_REFRESH_TOKEN)');
