...
export function liftRange(opts) {
  ...
  const aovFlag = (revPerClick < REV_PER_CLICK_MIN || revPerClick > REV_PER_CLICK_MAX)
    ...
    : { unverified: false };
  // 2026-05-26 defect D: booking conversion rate is the model's most
  // sensitive unverified input. Mark it `assumed` (amber UI) unless
  // the caller passes `conversionRateMeasured: true`. Revenue/click
  // scales linearly with conv rate, so a wrong assumption here cascades
  // through everything; surfacing it as flagged prevents the user from
  // mistaking the figure for a measured value.
  const convFlag = conversionRateMeasured
    ? { assumed: false }
    : { assumed: true, reason: 'Booking conversion rate is an assumed default — not measured. Provide a measured rate for this tier to remove this flag.' };
  const stack = [
    ...
    { label: 'Booking conversion rate',  value: conversionRate,   unit: 'frac', assumed: convFlag.assumed },
