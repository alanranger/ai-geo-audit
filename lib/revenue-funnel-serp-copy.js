...
export function buildSerpCopyAdvice({ pageUrl, rankingKw, rank, searchVolume, title, meta }) {
  ...
  if (isHubPage(url)) {
    ...
    return {
      ...
      reasons: ['Hub landing page: copy lists all course pathways, not one rank keyword.'],
      isHub: true
    };
  }
  ...
  if (actions.length === 0 && titleHasLead && metaLen >= META_MIN && metaLen <= META_MAX) {
    reasons.push('Title and meta length look acceptable; CTR lift may need FAQ/schema, not a full rewrite.');
  }
