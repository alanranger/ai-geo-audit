...
export function pageHeadNounFromUrl(url) {
  ...
  if (/hire-me|hire|professional-photographer/.test(p)) return 'hire';
...
/** Drop course/online queries on hire URLs (and similar cross-intent noise). */
export function keywordMatchesUrlIntent(keyword, url) {
  const k = String(keyword || '').toLowerCase();
  const noun = pageHeadNounFromUrl(url);
  if (noun === 'hire') {
    return /hire|photographer|headshot|portrait|commercial|event/.test(k)
      && !/\b(courses?|lessons?|workshops?|online)\b/.test(k);
  }
  if (noun === 'courses') return !/\bonline\b/.test(k);
  if (noun === 'online_course') return /\bonline\b/.test(k) || /\bfree\b.*\bcourse\b/.test(k);
  if (noun === 'workshops') return /workshop|near me|near-me/.test(k) || !/\b(courses?|coventry)\b/.test(k);
  return true;
}
...
export function pickKeywordForPage(cleanedUrl, keywords) {
  ...
  const intentOk = (k) => keywordMatchesUrlIntent(k.keyword, cleanedUrl);
  for (const ck of curated) {
    const hit = onPage.find((k) => String(k.keyword).toLowerCase() === ck && intentOk(k));
    if (hit) return hit;
  }
  let best = null;
  let bestVol = -1;
  for (const k of onPage) {
    if (!intentOk(k)) continue;
...
export function fitMetaDescription(raw, min = META_MIN, max = META_MAX) {
  ...
  if (s.length < min) {
    for (const extra of [
      ...
      const frag = normalizeSerpText(extra);
      if (s.toLowerCase().includes(frag.toLowerCase().trim())) continue;
      if (s.length + extra.length <= max) s = normalizeSerpText(s + extra);
    }
  }
  if (s.length > max) {
    let cut = s.lastIndexOf(' ', max);
    if (cut < min) cut = s.lastIndexOf(' ', min);
    s = normalizeSerpText((cut >= min ? s.slice(0, cut) : s.slice(0, max)).trimEnd());
    if (!/\.$/.test(s)) s += '.';
  }
  s = normalizeSerpText(s);
...
function titleLeadForPage(url, rankingKw) {
  ...
  if (noun === 'hire') {
    const hireLead = /hire|photographer in coventry|professional photographer/i.test(kw)
      ? rankingKw
      : 'Hire a Professional Photographer in Coventry';
    return {
      lead: hireLead,
      note: 'Commercial hire page — do not reuse courses hub keywords in title/meta.'
    };
  }
  return { lead: rankingKw, note: null };
}

/** One-line label + quoted string for dashboard “paste this” rows. */
export function pasteCopyLine(label, text) {
  const t = normalizeSerpText(text || '');
  if (!t) return '';
  return `Paste — ${label} (${t.length}ch): "${t}"`;
}

const HIRE_META_TEXT = 'Hire a professional photographer in Coventry - portraits, headshots, events and commercial shoots. View packages. Book a free consultation with Alan Ranger.';

export function proposeHireTitle(currentTitle) {
  const tailMatch = currentTitle && currentTitle.match(/\|\s*[^|]{5,50}$/);
  const tail = tailMatch ? normalizeSerpText(tailMatch[0]) : ' | Alan Ranger Photography';
  const core = 'Professional Photographer in Coventry';
  let s = normalizeSerpText(core + tail);
  if (serpLength(s) > TITLE_MAX) s = normalizeSerpText(core + ' | Alan Ranger');
  return s;
}

export function proposeHireH1() {
  return 'Professional Photographer in Coventry - Business & Portraits';
}

function buildHireSerpAdvice({ title, meta, h1, rankingKw, rank, searchVolume, note }) {
  const titleEx = proposeHireTitle(title);
  const metaBuilt = fitMetaDescription(HIRE_META_TEXT);
  const metaEx = metaBuilt.text;
  const h1Ex = proposeHireH1();
  const titleHasLead = title && /professional photographer/i.test(normalizeSerpText(title));
  const metaOk = serpLength(meta) >= META_MIN && serpLength(meta) <= META_MAX
    && /hire|professional photographer/i.test(normalizeSerpText(meta));
  const h1Ok = h1 && serpLength(h1) >= 40 && serpLength(h1) <= 70
    && /professional photographer/i.test(normalizeSerpText(h1));
  const actions = [];
  if (!titleHasLead) {
    actions.push({
      tag: 'title',
      confidence: 'high',
      headline: `Set SEO title (${serpLength(titleEx)}ch)`,
      detail: pasteCopyLine('Squarespace SEO title', titleEx)
    });
  }
  if (!metaOk) {
    actions.push({
      ...
      detail: pasteCopyLine('Squarespace SEO description', metaEx)
    });
  }
  if (!h1Ok) {
    actions.push({
      tag: 'h1',
      confidence: 'high',
      headline: `Set page H1 (${serpLength(h1Ex)}ch)`,
      detail: pasteCopyLine('Squarespace page H1 (main heading on page, not SEO title)', h1Ex)
    });
  }
  const serpComplete = actions.length === 0;
  return {
    blocked: false,
    lead: 'Professional Photographer in Coventry',
    titleExample: titleEx,
    metaExample: metaEx,
    metaExampleLength: metaBuilt.length,
    h1Recommendation: h1Ex,
    note: note || 'Commercial hire page — paste the three blocks below into Squarespace.',
    actions,
    reasons: serpComplete ? ['Hire page SERP copy is on target.'] : ['Use the paste blocks for title, meta and H1.'],
    isHub: false,
    serpComplete
  };
}
...
export function buildMetaSeed(url, lead, rankingKw) {
  ...
  if (head.toLowerCase() === kw.toLowerCase()) {
    return `Looking for ${head} in Coventry? Alan Ranger offers professional photography for portraits, events and commercial clients. Compare packages.`;
  }
  return `${head} - ${kw}. Compare options and packages.`;
}
...
export function buildSerpCopyAdvice({ pageUrl, rankingKw, rank, searchVolume, title, meta, h1 }) {
  const url = pageUrl || '';
  if (pageHeadNounFromUrl(url) === 'hire') {
    const { note } = titleLeadForPage(url, rankingKw);
    return buildHireSerpAdvice({ title, meta, h1, rankingKw, rank, searchVolume, note });
  }
  if (isHubPage(url)) {
  ...
  if (lead && !titleHasLead && example) {
    actions.push({
      tag: 'title',
      confidence: pageHeadNounFromUrl(url) === 'courses' && /lessons/i.test(String(rankingKw))
        ? 'medium'
        : 'high',
      headline: `Set SEO title (${serpLength(example)}ch)`,
      detail: pasteCopyLine('Squarespace SEO title', example)
    });
  }
  ...
  if (metaLen > META_MAX || metaLen < META_MIN || (meta && lead && metaNeedsLead(meta, lead))) {
    ...
    if (fitted.valid) {
      ...
      actions.push({
        ...
        detail: pasteCopyLine('Squarespace SEO description', fitted.text)
      });
