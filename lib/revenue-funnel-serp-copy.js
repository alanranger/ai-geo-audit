...
export function pageHeadNounFromUrl(url) {
  ...
  if (/hire|professional-photographer/.test(p)) return 'hire';
...
export function buildMetaSeed(url, lead, rankingKw) {
  const noun = pageHeadNounFromUrl(url);
  if (noun === 'hire') {
    return 'Hire a professional photographer in Coventry - portraits, headshots, events and commercial shoots. View packages and availability. Book a free consultation with Alan Ranger.';
  }
  if (noun === 'workshops') {
