...
export async function academyTierHealth(supabase, propertyUrl) {
  ...
  // 2026-05-26 SINGLE-SOURCE-OF-TRUTH FIX (Phase L): read from
  ...
  // stripe_supplemental Acuity row could overlap).
  //
  // 2026-05-26 Phase L1 correction: the verbatim "12. Academy" category is
  // the canonical value. Phase L's pre-rolled `tier_revenue.academy` field
  // is gone; we now read it out of the per-month `category_revenue` jsonb
  // (12 keys, verbatim from the Booking Sheet).
  const { data, error } = await supabase
    .from('booking_sheet_monthly_wide')
    .select('period_start, category_revenue')
    .eq('property_url', propertyUrl)
  ...
  const months = (data || []).map(row => {
    const rev = Number((row.category_revenue || {})['12. Academy']) || 0;
