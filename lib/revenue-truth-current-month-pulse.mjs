...
function trailing6SameDayRecurringAvg(txns, seasonalityByProduct, year, month, maxDay) {
  const vals = [];
  for (let i = 1; i <= 6; i++) {
    const p = shiftMonth(year, month, -i);
    vals.push(sumRecurringForMonth(txns, seasonalityByProduct, p.year, p.month, maxDay).recurringBaseline);
  }
  return vals.length ? round2(vals.reduce((a, b) => a + b, 0) / vals.length) : 0;
}
...
export function buildCurrentMonthPulse(txns, cfg, monthly, forecast, seasonalityByProduct = new Map()) {
  ...
  // Headline pulse basis follows the "Include JLR" toggle (cfg.includeJlr).
  // The recurring-baseline sub-block below is a separate lens and is unaffected.
  ...
  const trailing6RecurringAvg = trailing6SameDayRecurringAvg(txns, seasonalityByProduct, year, month, dayElapsed);
