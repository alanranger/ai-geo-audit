...
// Redemptions are always excluded (voucher double-counts). JLR is excluded only
// when includeJlr is not explicitly true — i.e. the "Include JLR" toggle drives it,
// matching the headline basis used by the rest of the Revenue Truth tab.
function isCountableTxn(t, includeJlr) {
  if (t.is_redemption) return false;
  if (includeJlr === true) return true;
  return !t.is_jlr;
}

function sumNonJlrThroughDay(txns, year, month, maxDay, includeJlr) {
  let s = 0;
  for (const t of txns) {
...
function trailing6SameDayAvg(txns, year, month, maxDay, includeJlr) {
  const vals = [];
  for (let i = 1; i <= 6; i++) {
    const p = shiftMonth(year, month, -i);
    vals.push(sumNonJlrThroughDay(txns, p.year, p.month, maxDay, includeJlr));
  }
  return vals.length ? round2(vals.reduce((a, b) => a + b, 0) / vals.length) : 0;
}

function trailing6SameDayRecurringAvg(txns, seasonalityByProduct, year, month, maxDay, includeJlr) {
  const vals = [];
  for (let i = 1; i <= 6; i++) {
    const p = shiftMonth(year, month, -i);
    vals.push(sumRecurringForMonth(txns, seasonalityByProduct, p.year, p.month, maxDay, includeJlr).recurringBaseline);
  }
  return vals.length ? round2(vals.reduce((a, b) => a + b, 0) / vals.length) : 0;
}

/** Full-month countable total for one calendar month (JLR per includeJlr). */
function sumNonJlrFullMonth(txns, year, month, includeJlr) {
  let s = 0;
  for (const t of txns) {
    if (!isCountableTxn(t, includeJlr)) continue;
    if (Number(t.year) !== year) continue;
...
export function trailing6SameCalendarMonthAvg(txns, year, month, includeJlr = false) {
  const vals = [];
  for (let i = 1; i <= 6; i++) {
    vals.push(sumNonJlrFullMonth(txns, year - i, month, includeJlr));
  }
...
/** Prior calendar year same month — primary blend anchor (avoids stale strong years). */
export function priorYearSameMonthFullNonJlr(txns, year, month, includeJlr = false) {
  return sumNonJlrFullMonth(txns, year - 1, month, includeJlr);
}
...
export const DEFCON_SURVIVAL_LINE = DEFAULT_TIER_BANDS.survival;

/** DEFCON cutoffs as fractions of the survival line (30% / 50% / 75% / 100%). */
const DEFCON_FRAC = [0.3, 0.5, 0.75, 1];

function defconCutoffs(survivalLine) {
  const s = Number(survivalLine) || DEFCON_SURVIVAL_LINE;
  return DEFCON_FRAC.map((f) => Math.round(s * f));
}

function buildDefconMeta(survivalLine) {
  const [t5, t4, t3, t1] = defconCutoffs(survivalLine);
  return [
    { level: 5, label: 'EXTREME', min: 0, max: t5 - 0.01, colour: '#dc2626', pips: 5, pulse: true },
    { level: 4, label: 'CRITICAL', min: t5, max: t4 - 0.01, colour: '#ea580c', pips: 4, pulse: false },
    { level: 3, label: 'SEVERE', min: t4, max: t3 - 0.01, colour: '#f59e0b', pips: 3, pulse: false },
    { level: 2, label: 'WARNING', min: t3, max: t1 - 0.01, colour: '#a3c04a', pips: 2, pulse: false },
    { level: 1, label: 'STABLE', min: t1, max: Infinity, colour: '#22c55e', pips: 1, pulse: false }
  ];
}

function matchDefconMeta(projected, survivalLine = DEFCON_SURVIVAL_LINE) {
  const metas = buildDefconMeta(survivalLine);
  for (const meta of metas) {
    if (projected >= meta.min && projected <= meta.max) return meta;
  }
  return metas[0];
}

export function formatDefconLegend(survivalLine = DEFCON_SURVIVAL_LINE) {
  const [t5, t4, t3, t1] = defconCutoffs(survivalLine);
  const gbp = (n) => `£${n.toLocaleString('en-GB')}`;
  return `DEFCON 5 <${gbp(t5)} · 4 ${gbp(t5)}–${gbp(t4 - 1)} · 3 ${gbp(t4)}–${gbp(t3 - 1)} · 2 ${gbp(t3)}–${gbp(t1 - 1)} · 1 ≥${gbp(t1)}`;
}
...
export function computeDefcon(projectedMonthEnd, survivalLine = DEFCON_SURVIVAL_LINE) {
  ...
  const meta = matchDefconMeta(projected, survival);
  return {
    level: meta.level,
    status: meta.label,
    colour: meta.colour,
...
function monthlyNonJlrTotals(txns, includeJlr) {
  const map = new Map();
  for (const t of txns) {
    if (!isCountableTxn(t, includeJlr)) continue;
...
function tierSoFar(txns, year, month, maxDay, includeJlr) {
  const out = new Map();
  for (const t of txns) {
    if (!isCountableTxn(t, includeJlr)) continue;
    if (Number(t.year) !== year) continue;
...
// Split per-tier current-vs-prior-year-same-day gaps into exec-summary tiers and
// volatile tiers. Extracted from buildCurrentMonthPulse to keep its complexity low.
function buildTierGaps(currentSoFar, priorSoFar) {
  const tierGaps = [];
  const volatileTiers = [];
  for (const tierKey of Object.keys(TIER_DEFINITIONS)) {
    ...
    const row = {
      ...
      gap_gbp: round2(cur - pri),
      gap_pct: pctChange(cur, pri)
    };
    if (VOLATILE_TIER_KEYS.has(tierKey)) volatileTiers.push(row);
    else if (EXEC_SUMMARY_TIER_KEYS.has(tierKey)) tierGaps.push(row);
  }
  tierGaps.sort((a, b) => Math.abs(b.gap_gbp) - Math.abs(a.gap_gbp));
  return { tierGaps, volatileTiers };
}
...
export function buildCurrentMonthPulse(txns, cfg, monthly, forecast, seasonalityByProduct = new Map()) {
  ...
  // Both the headline pulse and the recurring-baseline sub-block follow the
  // "Include JLR" toggle (cfg.includeJlr); recurring still strips lumpy tiers.
  const includeJlr = cfg?.includeJlr === true;

  const bookedSoFar = sumNonJlrThroughDay(txns, year, month, dayElapsed, includeJlr);
  const priorYearSameDay = sumNonJlrThroughDay(txns, year - 1, month, dayElapsed, includeJlr);
  const priorYearSameMonthFull = priorYearSameMonthFullNonJlr(txns, year, month, includeJlr);
  const trailing6Avg = trailing6SameDayAvg(txns, year, month, dayElapsed, includeJlr);
  const trailing6RecurringAvg = trailing6SameDayRecurringAvg(txns, seasonalityByProduct, year, month, dayElapsed, includeJlr);
  const trailing6MonthAvg = trailing6SameCalendarMonthAvg(txns, year, month, includeJlr);
  ...
  const history = monthlyNonJlrTotals(txns, includeJlr);
  ...
  const currentSoFar = tierSoFar(txns, year, month, dayElapsed, includeJlr);
  const priorSoFar = tierSoFar(txns, year - 1, month, dayElapsed, includeJlr);
  const { tierGaps, volatileTiers } = buildTierGaps(currentSoFar, priorSoFar);
  ...
  const recurringSoFar = sumRecurringForMonth(txns, seasonalityByProduct, year, month, dayElapsed, includeJlr);
  ...
  const closedYtdRecurringAvg = closedThisYear.length
    ? round2(closedThisYear.reduce((s, m) => s + (m.recurringBaseline || 0), 0) / closedThisYear.length)
    : null;
  ...
  return {
    ...
    recurring_baseline: {
      ...
      closed_ytd_avg: closedYtdRecurringAvg,
      closed_ytd_count: closedThisYear.length
    },
    ...
    comparisons: {
      ...
      trailing_6_same_day_recurring_avg: { ...compareMetric(recurringSoFar.recurringBaseline, trailing6RecurringAvg), rag: ragPartial(recurringSoFar.recurringBaseline), basis: 'recurring_baseline' },
    ...
    projection: {
      ...
      blend_anchor_label: (priorYearSameMonthFull > 0 ? 'Prior-year same month (full' : '6-yr same-month avg (')
        + (includeJlr ? ', JLR incl.)' : ', non-JLR)'),
