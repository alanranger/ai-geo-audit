...
const DEFCON_META = [
  { level: 5, label: 'EXTREME', min: 0, max: 1349.99, colour: '#dc2626', pips: 5, pulse: true },
  { level: 4, label: 'CRITICAL', min: 1350, max: 2249.99, colour: '#ea580c', pips: 4, pulse: false },
  { level: 3, label: 'SEVERE', min: 2250, max: 3349.99, colour: '#f59e0b', pips: 3, pulse: false },
  { level: 2, label: 'WARNING', min: 3350, max: 4449.99, colour: '#a3c04a', pips: 2, pulse: false },
  { level: 1, label: 'STABLE', min: 4450, max: Infinity, colour: '#22c55e', pips: 1, pulse: false }
];

function matchDefconMeta(projected) {
  for (const meta of DEFCON_META) {
    if (projected >= meta.min && projected <= meta.max) return meta;
  }
  return DEFCON_META[0];
}

export function formatDefconLegend() {
  return 'DEFCON 5 <£1,350 · 4 £1,350–£2,249 · 3 £2,250–£3,349 · 2 £3,350–£4,449 · 1 ≥£4,450';
}
...
export function computeDefcon(projectedMonthEnd, survivalLine = DEFCON_SURVIVAL_LINE) {
  ...
  const meta = matchDefconMeta(projected);
  return {
    level: meta.level,
    status: meta.label,
    colour: meta.colour,
