...
const DEFCON_META = [
  { level: 5, label: 'EXTREME', minPct: 0, maxPct: 25, colour: '#dc2626', pips: 5, pulse: true },
  { level: 4, label: 'CRITICAL', minPct: 25, maxPct: 50, colour: '#ef4444', pips: 4, pulse: false },
  { level: 3, label: 'SEVERE', minPct: 50, maxPct: 75, colour: '#f59e0b', pips: 3, pulse: false },
...
export function computeDefcon(projectedMonthEnd, survivalLine = DEFCON_SURVIVAL_LINE) {
  ...
  if (pctOfSurvival < 25) meta = DEFCON_META[0];
  else if (pctOfSurvival < 50) meta = DEFCON_META[1];
...
export function buildDefconGauge(dayElapsed, blendedProjection, survivalLine = DEFCON_SURVIVAL_LINE) {
  if (dayElapsed < 5) {
    return {
      active: false,
      placeholder: 'Insufficient data — too early in month for projection (DEFCON from day 5).'
    };
  }
  const defcon = computeDefcon(blendedProjection, survivalLine);
  return { active: true, ...defcon, exec_worry: defcon.level >= 3 };
}
...
export function buildCurrentMonthPulse(txns, cfg, monthly, forecast) {
  const now = cfg?.now || {};
  ...
  const blendedMonthEnd = blendedMonthEndProjection(bookedSoFar, dayElapsed, dim, trailing6MonthAvg);
  const projectedMonthEnd = blendedMonthEnd;
  const projectedBand = classifyBand(projectedMonthEnd, bands);
  const blendWeights = blendProjectionWeights(dayElapsed);
  const defcon = buildDefconGauge(dayElapsed, blendedMonthEnd, bands.survival);
  ...
  const leadMessage = defcon.active && defcon.level >= 3
    ? `${monthName(year, month)} projects ${fmtPlain(projectedMonthEnd)} (${defcon.pct_of_survival.toFixed(0)}% of £${bands.survival.toLocaleString('en-GB')} survival) — DEFCON ${defcon.level} ${defcon.status}.`
    : (gapSurvival < 0
      ? `${monthName(year, month)} is tracking ${fmtPlain(Math.abs(gapSurvival))} below survival on blended projection.`
      : `${monthName(year, month)} is above survival on current blended projection.`);

  const revised = buildRevisedForecast(
    monthly, cfg, forecast, projectedHeadline(monthly, cfg, dayElapsed, dim), blendedMonthEnd
  );
  ...
  return {
    ...
    defcon,
    comparisons: {
      prior_year_same_month: { ...compareMetric(bookedSoFar, priorYearSameDay), rag: ragPartial(bookedSoFar) },
      trailing_6_same_day_avg: { ...compareMetric(bookedSoFar, trailing6Avg), rag: ragPartial(bookedSoFar) },
      comfortable_pro_rata: { target: comfortableProRata, ...compareMetric(bookedSoFar, comfortableProRata), rag: ragPartial(bookedSoFar) }
    },
    projection: {
...
function projectedHeadline(monthly, cfg, dayElapsed, dim) {
  const year = cfg?.now?.year;
  const month = cfg?.now?.month;
  const partial = (monthly || []).find((m) => m.year === year && m.month === month && m.isPartial);
  const h = partial ? partial.headlineRevenue : 0;
  return dayElapsed > 0 ? round2((h / dayElapsed) * dim) : round2(h);
}

function buildRevisedForecast(monthly, cfg, forecast, projectedCurrentHeadline, blendedMonthEnd) {
  ...
  const revisedLinear = round2(ytdClosed + projectedCurrentHeadline + avg * monthsAfterCurrent);
  const revisedBlended = blendedMonthEnd != null
    ? round2(ytdClosed + blendedMonthEnd + avg * monthsAfterCurrent)
    : null;
  const primary = revisedBlended ?? revisedLinear;
  return {
    current_forecast: forecast.forecastCentral,
    revised_forecast: revisedLinear,
    revised_forecast_blended: revisedBlended,
    revised_forecast_primary: primary,
    delta_gbp: round2(primary - forecast.forecastCentral),
    delta_linear_gbp: round2(revisedLinear - forecast.forecastCentral),
    current_label: 'Closed-months-only (trailing-3 × remaining)',
    revised_label: 'Incl. current-month linear projection',
    revised_blended_label: 'Incl. current-month blended projection (recommended)'
  };
}
...
export function computePulseGscSignals(diagnosis) {
  ...
  for (const t of diagnosis?.tier_rollup || []) {
    ...
    for (const s of t.product_gsc_trend?.slugs || []) {
      ...
      productRows.push({ slug: s.slug, tier_key: t.tier_key, impressions: imp, delta_pct: d });
    }
  }
...
export function liveMonthWorryText(pulse) {
  ...
  return `🚨 DEFCON ${d.level} — ${mo} tracking ${fmtPlain(d.projected_month_end)} projected (${d.pct_of_survival.toFixed(0)}% of survival).${worst}`;
}
