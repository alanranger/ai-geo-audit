...
//
// Tiers in TIER_GSC_ROLE_OVERRIDES replace the canonical derivation entirely
// (commissions, pick_n_mix_inc, academy — Acuity/invoicing or business split).
//
...
/** Post-consolidation hub remaps ({ fromSlug: toSlug }). Empty until hub work ships. */
export const NAV_HUB_SLUG_OVERRIDES = Object.freeze({});

const PSM = 'photography-services-near-me';

/**
 * Explicit L1/L3 roles replacing canonical_products for these tiers.
 * Edit here when GSC overlay scope changes (not revenue booking categories).
 */
export const TIER_GSC_ROLE_OVERRIDES = Object.freeze({
  ...
  pick_n_mix_inc: {
    nav_hub_slugs: ['photography-payment-plan'],
    product_slugs: [
      `${PSM}/monthly-pick-n-mix-subscription`,
      `${PSM}/quarterly-pick-n-mix-subscription`,
      `${PSM}/annual-pick-n-mix-subscription`
    ]
  },
  academy: {
    nav_hub_slugs: ['free-online-photography-course'],
    product_slugs: [
      `${PSM}/camera-settings-photography-field-checklists`,
      `${PSM}/composition-settings-photography-field-checklists`,
      `${PSM}/foundation-digital-pack-plus`,
      `${PSM}/photography-35bundle-photography-field-checklists`,
      `${PSM}/photography-foundation-course-ebook`,
      `${PSM}/photography-genre-photography-field-checklists`,
      `${PSM}/photography-tips-pocket-guide-series`,
      `${PSM}/02-top-10-tips-to-make-a-photograph`,
      `${PSM}/04-top-10-tips-to-better-composition`,
      `${PSM}/05-top-10-tips-to-better-portraits`,
      `${PSM}/06-top-10-tips-to-better-landscapes`,
      `${PSM}/07-top-10-tips-to-better-bw-photos`
    ]
  }
});
...
export function resolveNavHubSlug(slug) {
  ...
  if (!n) return null;
  return normalizePageSlug(NAV_HUB_SLUG_OVERRIDES[n] || n);
}
