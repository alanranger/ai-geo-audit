...
//
// This LEGACY parser walked the Booking Sheet's transactional rows and
// applied a funding filter (Bank+PayPal+Cash kept; Stripe dropped) on the
// assumption that the dashboard would also be summing the Squarespace API
// + Stripe API sources to make up the total. That assumption was wrong:
// the three sources overlap (a Squarespace order paid by Bank Transfer
// appears in BOTH the SQ Orders API row AND the Booking Sheet's Bank
// receipt row) with no transaction-level dedup, so headline figures were
// double-counted.
//
// REPLACED BY: `lib/booking-sheet-truth-parser.mjs` (reads row-18 Totals
// from each "Sales YYYY" tab -- the user-maintained master figure that
// already includes every funding channel).
//
// This file is preserved only to:
//   - decode the legacy import shape for historic spreadsheet readers
//   - avoid breaking any external script that still imports it
//
// Do NOT call `parseBookingSheet` from new code. Use
// `parseBookingSheetTruth` from `lib/booking-sheet-truth-parser.mjs`.
//
...
//   - PicknMix            -> SKIP since 18 May 2026: the SQ sync now detects
//                            PICKNMIX 100%-discount orders and does the same
//                            debit/credit reallocation deterministically from
//                            the order line items. Including these rows here
//                            would double-count the reallocation.
//
...
export const COMMERCIAL_TIER_IDS = ['workshops_residential', 'workshops_nonres', 'courses', 'services', 'hire', 'academy', 'unidentified'];

// Funding values we INCLUDE in the booking-sheet importer. Lowercased
// for comparison. Anything else (notably 'stripe', and 'picknmix' which
// the SQ sync now handles natively) is skipped.
...
export function classifyCategory(rawCategory) {
  ...
  // Workshops: the spreadsheet pre-classifies Residential vs Non-Residential
  // by row, which is the most authoritative signal we have.
  if (lc.includes('workshop')) {
    if (lc.includes('residential') && !lc.includes('non')) return 'workshops_residential';
...
/** Sum verbatim Booking Sheet category_revenue into legacy 6-tier keys for money-page tiles. */
export function synthesiseLegacyTierRevenue(categoryRevenue) {
  const cats = categoryRevenue || {};
  const totals = {};
  for (const [label, rawVal] of Object.entries(cats)) {
    const tier = classifyCategory(label);
    const v = Number(rawVal);
    if (!tier || tier === 'unidentified' || !Number.isFinite(v)) continue;
    totals[tier] = (totals[tier] || 0) + v;
  }
  const pick = (id) => (totals[id] != null ? Math.round(totals[id] * 100) / 100 : null);
  return {
    courses: pick('courses'),
    workshops_nonres: pick('workshops_nonres'),
    workshops_residential: pick('workshops_residential'),
