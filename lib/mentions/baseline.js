...
const parseSeedMapFromJson = (raw) => {
  try {
    ...
    const output = {};
    for (const [platformRaw, value] of Object.entries(parsed)) {
      const platform = normalizeKeyword(platformRaw);
      if (!platform) continue;
      const urls = Array.isArray(value) ? value : [value];
      output[platform] = urls.map((item) => String(item || '').trim()).filter(Boolean);
    }
    return output;
  } catch (err) {
    return {};
...
const parseSeedMapFromLines = (raw) => {
  ...
  for (const line of lines) {
    ...
    if (idx <= 0) continue;
    const platform = normalizeKeyword(line.slice(0, idx));
    const url = String(line.slice(idx + 1) || '').trim();
    if (!platform || !url) continue;
    output[platform] = output[platform] || [];
    output[platform].push(url);
  }
  return output;
};

const parsePlatformSeedMap = (raw) => {
  const fromJson = parseSeedMapFromJson(raw);
  if (Object.keys(fromJson).length) return fromJson;
  return parseSeedMapFromLines(raw);
};

const resolveSeedMentions = (seedMap) => {
  const rows = [];
  for (const [platform, urls] of Object.entries(seedMap || {})) {
    const domain = MENTIONS_PLATFORM_DOMAINS[platform];
    if (!domain || !Array.isArray(urls)) continue;
    const valid = uniq(urls).filter((url) => hostMatchesDomain(extractHostname(url), domain));
    for (const url of valid) {
      rows.push({
        platform,
        sourceDomain: extractHostname(url) || domain,
        sourceUrl: url,
        title: 'Manual seeded social URL',
        snippet: 'manual_seed',
        publishedAt: null,
        matchedKeyword: 'manual_seed',
        isSeed: true
      });
    }
  }
...
const getEffectiveScoreMeta = (row, scoreMeta) => {
  if (!row?.isSeed) return scoreMeta;
  return { mentionScore: 65, alertLevel: 'watch', isBrandMention: true, isStrongBrandMention: true };
};

const reduceMentions = (rawMentions, brandTerms) => {
  ...
  for (const row of rawMentions) {
    ...
    const effectiveScoreMeta = getEffectiveScoreMeta(row, scoreMeta);
    if (!effectiveScoreMeta.isStrongBrandMention) continue;
    if (!existing) {
      deduped.set(key, buildMentionSeed({ row, host, platform: derivedPlatform, scoreMeta: effectiveScoreMeta }));
      continue;
    }
    deduped.set(key, mergeMentionRows(existing, row, effectiveScoreMeta));
  }
...
const normalizeUrlForKey = (url) => {
  try {
    ...
    if (hostMatchesDomain(host, 'linkedin.com')) parsed.hostname = 'linkedin.com';
    if (hostMatchesDomain(host, 'youtube.com')) parsed.hostname = 'youtube.com';
    if (hostMatchesDomain(host, 'reddit.com')) parsed.hostname = 'reddit.com';
    parsed.hash = '';
...
const hasTermHit = (term, title, snippet, url) => (
  title.includes(term) || snippet.includes(term) || url.includes(term)
);

const applyWeakBrandSignals = ({ weakTerms, title, snippet, haystackUrl, hasContextSignal }) => {
  let score = 0;
  let hasBrand = false;
  ...
  for (const termRaw of weakTerms) {
    ...
    if (!term) continue;
    if (!hasTermHit(term, title, snippet, haystackUrl)) continue;
    score += 5;
    hasBrand = true;
    if (hasContextSignal) {
      score += 20;
...
const applyStrongBrandSignals = ({ terms, title, snippet, haystackUrl }) => {
  let score = 0;
  let hasBrand = false;
  let hasStrongBrand = false;
...
const applyBrandTermScore = (
  ...
  const weakMeta = applyWeakBrandSignals({ weakTerms, title, snippet, haystackUrl, hasContextSignal });
  const strongMeta = applyStrongBrandSignals({ terms, title, snippet, haystackUrl });
  return {
    score: weakMeta.score + strongMeta.score,
    hasBrand: weakMeta.hasBrand || strongMeta.hasBrand,
    hasStrongBrand: weakMeta.hasStrongBrand || strongMeta.hasStrongBrand
  };
};
...
export const collectMentionCandidates = async ({
  ...
  const seedMap = parsePlatformSeedMap(platformSeedMapRaw);
  const seedMentions = resolveSeedMentions(seedMap);
  ...
  rawMentions.push(...seedMentions);

  const mentions = reduceMentions(rawMentions, brandTerms);
