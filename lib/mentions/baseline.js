...
const DEFAULT_BRAND_TERMS = ['alan ranger', 'alan ranger photography', 'alanranger.com'];
...
const applyBrandTermScore = (terms, title, snippet) => {
  let score = 0;
  let hasBrand = false;
...
const computeMentionScore = ({ title, snippet, keyword, brandTerms, publishedAt }) => {
  const haystackTitle = normalizeKeyword(title);
  const haystackSnippet = normalizeKeyword(snippet);
  const brandMeta = applyBrandTermScore(brandTerms, haystackTitle, haystackSnippet);
...
const fetchMentionsForPair = async ({ platform, domain, keyword, brandTerm, perQueryLimit }) => {
  const query = `site:${domain} "${keyword}" "${brandTerm}"`;
  const url = `https://www.bing.com/search?q=${encodeURIComponent(query)}&format=rss&setlang=en-gb`;
  const response = await fetch(url, {
    headers: { 'user-agent': 'Mozilla/5.0 (compatible; AI-GEO-Audit/mentions-baseline)' }
  });
  if (!response.ok) {
    throw new Error(`Bing RSS failed (${platform}:${keyword}) HTTP ${response.status}`);
  }
  const xml = await response.text();
  const items = parseBingRssItems(xml).slice(0, perQueryLimit);
...
export const collectMentionCandidates = async ({
  ...
  const keywordsUsed = normalizedKeywords.slice(0, Math.max(1, Number(keywordLimit || DEFAULT_KEYWORD_LIMIT)));
  const brandTerm = normalizeKeyword(brandTerms[0] || DEFAULT_BRAND_TERMS[0]);
  const pairs = [];

  for (const keyword of keywordsUsed) {
    for (const [platform, domain] of Object.entries(MENTIONS_PLATFORM_DOMAINS)) {
      pairs.push({ platform, domain, keyword, brandTerm, perQueryLimit });
    }
  }
  ...
  for (const row of rawMentions) {
    ...
    const key = `${row.platform}|${normalizeUrlForKey(row.sourceUrl)}`;
    ...
    const keywordsMerged = uniq([...(existing.matched_keywords || []), row.matchedKeyword].filter(Boolean));
    const useIncoming = scoreMeta.mentionScore > Number(existing.mention_score || 0);
    deduped.set(key, {
      ...existing,
      title: useIncoming ? (row.title || existing.title || null) : existing.title,
      snippet: useIncoming ? (row.snippet || existing.snippet || null) : existing.snippet,
      published_at: existing.published_at || row.publishedAt || null,
