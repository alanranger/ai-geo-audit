...
function parseDfsDate(s) {
  ...
  if (!norm.includes('T')) {
    norm = norm.replace(/^(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2}:\d{2})/, '$1T$2');
  }
  // DataForSEO often returns "2024-09-18T19:43:38 +00:00" (space before offset).
  norm = norm.replace(/ ([+-]\d{2}:\d{2})$/, '$1');
 ...
 * @param {(string|[string,string,string])[]|null|undefined} filters DataForSEO `filters`; omit/null/[] = no URL filter (audit).
...
export async function paginateDomainBacklinks(creds, domainHost, filters, runId) {
  ...
  return { rows, pages, totalCost, maxFirstSeen, truncated, itemsFromApi };
}
