...
(function () {
  ...
  function deltaMarker(youVal, themVal) {
    if (youVal == null || themVal == null) return '';
    if (themVal > youVal) return ' <span class="ca-delta ca-delta--down">▼</span>';
    if (themVal < youVal) return ' <span class="ca-delta ca-delta--up">▲</span>';
    return '';
  }
  ...
  function renderAuthorityTile(opts) {
    ...
    if (measured && snap?.score != null) {
      ...
      rows.push(`<div class="ca-auth-row"><span>Domain strength</span><strong>${fmtNum(them)}${isYou ? '' : deltaMarker(you, them)}</strong></div>`);
    } else if (!isYou && domain?.includes('.')) {
      rows.push(`<div class="ca-auth-row"><span>Domain strength</span><em>n/a</em> <button type="button" class="btn btn-small ca-queue-btn" data-domain="${domain}">Queue snapshot</button></div>`);
    }
    if (measured && snap?.top3_keywords_raw != null) {
      ...
      rows.push(`<div class="ca-auth-row"><span>Top-3 keywords</span><strong>${fmtNum(them)}${isYou ? '' : deltaMarker(you, them)}</strong></div>`);
    }
    if (measured && snap?.top10_keywords_raw != null) {
      ...
      rows.push(`<div class="ca-auth-row"><span>Top-10 keywords</span><strong>${fmtNum(them)}${isYou ? '' : deltaMarker(you, them)}</strong></div>`);
    }
    if (!isYou) {
      if (review?.review_count != null) {
        rows.push(`<div class="ca-auth-row"><span>GBP reviews</span><strong>${review.review_count} · ${review.rating ?? '?'}★</strong></div>`);
      } else if (review && review.reason === 'no_pack_match_or_rating') {
        rows.push('<div class="ca-auth-row"><span>GBP reviews</span><em>no GBP listing</em></div>');
      }
      if (page?.title) {
        const title = String(page.title).slice(0, 40) + (page.title.length > 40 ? '…' : '');
        rows.push(`<div class="ca-auth-row"><span>On-page</span><strong>${title} (${page.word_count || '?'}w)</strong></div>`);
      }
    }
    ...
    return `<div class="ca-vs-card${isYou ? ' ca-vs-you' : ''}">
      ...
      ${rows.join('') || '<p class="ca-vs-empty">No authority data yet.</p>'}
      ${suggestBtn}
    </div>`;
  }
  ...
  function renderTable(rows, state) {
    ...
    if (!rows.length) {
      body.innerHTML = '<tr><td colspan="7" class="ranking-table-empty">No keywords match the current filters.</td></tr>';
      return;
    }
  ...
  async function renderDetail(state) {
    ...
    if (title) title.textContent = `Keyword detail — ${kw}`;
