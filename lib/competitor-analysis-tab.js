...
(function () {
  ...
  const MONEY_CLASSES = ['local-money', 'regional-money', 'national-money'];
  ...
  function showRenderError(message) {
    const msg = message || 'Could not load Competitor Analysis.';
    ...
    if (body) {
      body.innerHTML = `<tr><td colspan="13" class="ranking-table-empty">${msg}</td></tr>`;
    }
    const lb = document.getElementById('ca-leaderboard-body');
    if (lb) {
      lb.innerHTML = `<tr><td colspan="6" class="ranking-table-empty">${msg}</td></tr>`;
    }
  }
  ...
  function classPillHtml(cls) {
    ...
    const short = c === 'local-money' ? 'Local' : c === 'regional-money' ? 'Regional' : c === 'national-money' ? 'National' : c === 'brand' ? 'Brand' : c;
  ...
  async function renderCompetitorAnalysisTab() {
    ...
    try {
      ...
      const universe = buildUniverseRows(byKeyword, combinedRows, {}, state.independentsOnly);
      ...
      if (countEl) {
        countEl.textContent = `${universe.length} tracked keywords from locked-config · ${filtered.length} match filters · enriching rival metrics…`;
      }
      ...
      const enrichedUniverse = buildUniverseRows(byKeyword, combinedRows, meta, state.independentsOnly);
      await enrichRivalColumns(enrichedUniverse, meta, combinedRows);
      if (gen !== renderGen) return;

      state.universeRows = enrichedUniverse;
      filtered = sortRows(applyFilters(enrichedUniverse, state), state.sortColumn, state.sortDir);
      state.tableRows = filtered;
      renderTable(filtered, state);
      if (countEl) {
      ...
      await Promise.all([
        renderClassificationQueue(meta),
        renderCompetitorLeaderboard(state, combinedRows),
      ]);
      if (gen !== renderGen) return;
      ...
      if (gen !== renderGen) return;
      showRenderError(`Competitor Analysis failed to load: ${e?.message || 'unknown error'}`);
    }
  }
