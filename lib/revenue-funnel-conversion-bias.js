...
/**
 * Accepts either a ga4AttributedView() (bot-excluded, preferred) or a raw
 * ga4_site_metrics_28d row, so callers can migrate without a flag day.
 */
function enquiryPair(ga4Snap) {
  if (!ga4Snap) return { money: null, site: null };
  if (ga4Snap.money_page_enquiry_events != null || ga4Snap.enquiry_events != null) {
    return { money: Number(ga4Snap.money_page_enquiry_events), site: Number(ga4Snap.enquiry_events) };
  }
  return {
    money: Number(ga4Snap.money_page_enquiry_events_28d),
    site: Number(ga4Snap.enquiry_events_28d)
  };
}

export function conversionHealthFromMetrics(ga4Snap, revenueSnap) {
  const { money: moneyEnquiry, site: siteEnquiry } = enquiryPair(ga4Snap);
...
function leverMap(weights) {
  const raw = weights && weights.lever;
