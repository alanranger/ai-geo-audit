...
import { defaultLockedCsvPath, loadByKeywordFromCsv } from '../keyword-ranking/locked-config-merge.js';
...
function loadFlaggedPromptBank() {
  const byKeyword = loadByKeywordFromCsv(defaultLockedCsvPath(ROOT));
  const prompts = Object.values(byKeyword)
  ...
  return {
    version: 2,
    source: 'locked-config-llm_prompt',
    domain: DOMAIN,
    prompts,
...
export async function collectLlmVisibilitySnapshot(opts = {}) {
  const bank = loadFlaggedPromptBank();
