 ...
 * follow split, referring domains, ref. domain rank bands, dofollow by page tier (segmentation CSV + heuristic).
 */
...
} from '../api/aigeo/tier-segmentation.js';
...
export async function aggregateDfsBacklinkTileStats(supabase, domainHost) {
  ...
  const tierLookup = buildTierLookupFromEntries(entries);
  ...
  while (totalRows < MAX_SCAN) {
    ...
    for (let i = 0; i < data.length; i += 1) {
      ...
      if (row?.dofollow === true) {
        const pt = getTierForUrlFromLookup(row?.url_to, tierLookup, dom, true);
