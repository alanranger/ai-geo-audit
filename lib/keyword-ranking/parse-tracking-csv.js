const FULL_HEADER = ['keyword', 'tracking_location', 'location_name_dfs', 'class', 'target_page', 'llm_prompt'];

/** Strip Gumloop / Drive UTF-8 BOM; accept CRLF or LF (split on /\r?\n/). */
...
function isFullHeader(fields) {
  ...
  const norm = fields.map((f) => String(f || '').trim().replace(/^\uFEFF/, '').toLowerCase());
...
function rowFromFullFields(fields) {
  ...
  const llm = String(fields[5] || '').trim().toLowerCase();
  return {
    keyword,
    ...
    llm_prompt: llm === 'true' || llm === '1' || llm === 'yes',
  };
}
...
export function rowToCsvLine(row) {
  ...
  return [
    ...
    row.target_page || '',
    row.llm_prompt === true ? 'true' : 'false',
  ].join(',');
}
