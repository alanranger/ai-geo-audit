...
/** v11 — current locked set; CSV is fallback only (bundled JSON is primary). */
export function defaultLockedCsvPath(root) {
  const v11 = join(root, 'config/keyword-tracking-locations-and-class-LOCKED-v11.csv');
  if (!existsSync(v11)) {
    throw new Error(`Locked config CSV not found: ${v11}`);
  }
  return v11;
}
...
export function tryLoadByKeywordFromCsv(root) {
  const v11 = join(root, 'config/keyword-tracking-locations-and-class-LOCKED-v11.csv');
  if (!existsSync(v11)) return null;
  return loadByKeywordFromCsv(v11);
}
...
export function loadLockedByKeywordFromRepo(root) {
  ...
  throw new Error('No locked keyword config in bundled JSON or on-disk LOCKED-v11 CSV');
}
