...
export function extractLocalPackPosition(items, targetRoot = OUR_DOMAIN_ROOT) {
  const pack = (items || []).find((it) => it && it.type === 'local_pack');
  if (!pack) return { local_pack_present: false, local_pack_position: null };

  const businesses = Array.isArray(pack.items) ? pack.items : [];
  for (const biz of businesses) {
    const title = biz?.title || biz?.domain || '';
    const domain = biz?.domain || biz?.url || '';
    const ours =
      isOurBrandTitle(title) || isOurDomain(domain, targetRoot);
    if (!ours) continue;
    const pos =
      biz.rank_group != null
