...
export const buildCombinedRows = (serpRows, aiRows) => {
  ...
  return serpRows.map((row) => {
    ...
    return {
      ...
      location_code: row?.location_code ?? loc.location_code ?? null,
      location_coordinate: row?.location_coordinate
        ?? (loc.tier === 'L' ? getHyperlocalCoordinate() : null),
      device: row?.device ?? (loc.tier === 'L' ? 'desktop' : 'desktop'),
      os: row?.os ?? 'windows',
      serp_surface_stack: row?.serp_surface_stack ?? null,
      local_grid: row?.local_grid ?? null,
