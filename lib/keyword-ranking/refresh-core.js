...
const DEFAULT_DEPTH = 100; // matches serp-rank-test.js DEFAULT_SERP_DEPTH (2026-04-17)
...
export const fetchSerpRows = async (
  ...
  async (batch) => {
    const serpResp = await fetchJson(
      `${baseUrl}/api/aigeo/serp-rank-test?keywords=${encodeURIComponent(batch.join(','))}&depth=${depth}`
    );
