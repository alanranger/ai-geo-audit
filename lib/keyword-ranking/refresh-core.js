...
import { classifyKeywordSegment } from '../segment/classifyKeywordSegment.js';
...
export const buildCombinedRows = (serpRows, aiRows) => {
  ...
  return serpRows.map((row) => {
    ...
    const segmentOverride = resolveSegmentOverride(keyword);
    const segmentInfo = segmentOverride
      ? { segment: segmentOverride, confidence: 1, reason: 'manual: Alan segment ruling 2026-07-14' }
    ...
    return {
      ...
      segment_source: segmentOverride ? 'manual' : 'auto',
