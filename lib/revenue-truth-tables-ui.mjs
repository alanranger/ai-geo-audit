...
export function renderHeadlineStripHtml(strip, cfg, pulse) {
  ...
  const latestMeta = latest
    ? `${monthLabel(latest.year, latest.month)} · ${BAND_LABEL[latest.band]}${superseded}`
    : '';
  const cards = [
    stripCard('Latest closed month', latest ? fmtMoney(latest.headlineRevenue, 0) : '—', latestMeta, latest?.band, 'headline_gross'),
...
export function renderTierChartTable(monthlyAll, keys) {
  ...
  return `<div class="rt-basis-note">${basisBadge('headline_gross')} Monthly headline gross (12-category, JLR-inclusive).</div>`
    + `<table class="rt-table rt-striped"><thead><tr><th>Month</th><th>Headline £</th><th>Band</th><th></th></tr></thead><tbody>${rows}</tbody>${foot}</table>`;
}
