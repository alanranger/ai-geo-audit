...
import {
  headlineSignals, forecastSignals, channelSignals, clientsSignals, moversCardSignals
...
export function renderForecastHtml(forecast) {
  ...
  return signals
    + `<div class="rt-basis-note">${basisBadge('closed_only')} YTD ${fmtMoney(forecast.ytdActual, 0)} from ${forecast.closedMonths} closed months — same basis as tier rollup non-JLR.</div>`
    + `<div class="rt-forecast-grid">`
    + `<div class="rt-forecast-card"><div class="rt-fc-label">YTD actual</div><div class="rt-fc-value">${fmtMoney(forecast.ytdActual, 0)}</div></div>`
    + `<div class="rt-forecast-card"><div class="rt-fc-label">Full-year forecast</div><div class="rt-fc-value">${fmtMoney(central, 0)}</div><div class="rt-fc-meta">${fmtMoney(forecast.forecastLow, 0)} – ${fmtMoney(forecast.forecastHigh, 0)}</div></div>`
...
export function renderMarketTable(monthlyAll, keys) {
  ...
  return `<table class="rt-table rt-striped">${head}<tbody>${rows}</tbody></table>`;
}
...
export function renderCategoryTable(catsAll, keys, sortMode = 'market') {
  ...
  return `<table class="rt-table rt-striped">${head}<tbody>${body}</tbody></table>`;
}
...
export function renderProductBreakdownTable(findings, options = {}) {
  ...
  return `<div class="rt-basis-note">${basisBadge('nonjlr_net')} Product revenue — JLR excluded by default.</div><table class="rt-table rt-striped rt-breakdown-table">${head}<tbody>${body}</tbody></table>`;
}
...
export function renderPageBreakdownTable(findings, options = {}) {
  ...
  return `<div class="rt-basis-note">${basisBadge('nonjlr_gross_voucher')} Gross of voucher redemption — differs from headline where vouchers have no landing page.</div><table class="rt-table rt-striped rt-breakdown-table">${head}<tbody>${body}</tbody></table>`;
}
...
export function renderFundingTable(funding, keys) {
  ...
  return `<div class="rt-basis-note">${basisBadge('nonjlr_net')} Estimated fees from funding mix.</div><table class="rt-table rt-striped">${head}<tbody>${body}</tbody></table>`;
}
...
function renderMoverCard(f, findings, window, includeJlr) {
  ...
  const table = `<table class="rt-movers-mini-table"><thead><tr><th>Year</th><th>£</th></tr></thead><tbody>`
    ...
    + `<tr><td>${findings.currentYear} ann.</td><td>${fmtMoney(s.y2026_annualised, 0)}</td></tr></tbody></table>`;
...
export function renderTierChartTable(monthlyAll, keys) {
  ...
  return `<table class="rt-table rt-striped"><thead><tr><th>Month</th><th>Headline £</th><th>Band</th><th></th></tr></thead><tbody>${rows}</tbody></table>`;
}
