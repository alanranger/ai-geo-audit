...
export function renderHeadlineForecastPanelHtml(strip, cfg, forecast, pulse, recurringForecast, findings) {
  ...
  if (recFc.forecastCentral != null) {
    altMethods.push({ label: 'Recurring run-rate (no lumpy assumed)', value: recFc.forecastCentral, meta: `Trailing-3 recurring avg ${fmtMoney(recFc.runRateMonthly || 0, 0)}/mo · Excludes workshops residential & event-bound` });
  }
...
export function renderTierChartSvg(monthly, cfg, includeJlr) {
  ...
  const maxVal = Math.max(bands.thrive * 1.15, ...monthly.map((m) => Math.max(m.headlineRevenue || 0, m.recurringBaseline || 0)));
  const w = Math.max(720, monthly.length * 56 + 80);
  const h = 320;
  const padL = 48;
  const padR = 16;
  const padT = 28;
  ...
  const refLines = [
    { v: bands.survival, label: '£3k', cls: 'survival' },
    { v: bands.comfortable, label: '£5k', cls: 'comfortable' },
    { v: bands.thrive, label: '£8k', cls: 'thrive' }
  ];
  ...
  const bars = monthly.map((m, i) => {
    ...
    return `<g class="rt-chart-month" transform="translate(${cx},0)">`
      ...
      + `<rect class="rt-chart-bar-head" x="${-barW - 2}" y="${yHead}" width="${barW}" height="${hHead}" fill="${headCol}" rx="2" />`
      + `<rect class="rt-chart-bar-rec" x="2" y="${yRec}" width="${barW}" height="${hRec}" fill="url(#rt-hatch)" stroke="${BAND_COLOURS[m.recurringBand] || '#475569'}" stroke-width="1" rx="2" />`
      + `<text class="rt-chart-val" x="${-barW / 2 - 2}" y="${yHead - 4}" text-anchor="middle">${fmtMoney(head, 0)}</text>`
      + `<text class="rt-chart-val rt-chart-val-rec" x="${barW / 2 + 2}" y="${yRec - 4}" text-anchor="middle">${fmtMoney(rec, 0)}</text>`
      + `<text class="rt-chart-month-label" x="0" y="${h - 12}" text-anchor="middle">${lbl}</text>`
      + `</g>`;
  }).join('');

  const headlineNote = includeJlr ? 'Headline (JLR incl.)' : 'Headline (JLR excluded)';
  return `<div class="rt-tier-chart-inner" style="width:${w}px"><svg class="rt-tier-chart-svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMinYMid meet" role="img" aria-label="Monthly headline vs recurring baseline">`
