...
export function renderHeadlineForecastPanelHtml(strip, cfg, forecast, pulse, recurringForecast) {
  ...
  const recBlock = recFc.forecastCentral != null
    ? `<div class="rt-hf-recurring"><div class="rt-hf-subhead">Recurring baseline forecast ${basisBadge('recurring_baseline')}</div>`
      + `<div class="rt-forecast-grid">`
      + forecastScenarioCard('Recurring run-rate (no lumpy assumed)', recFc.forecastCentral, `Trailing-3 recurring avg ${fmtMoney(recFc.runRateMonthly || 0, 0)}/mo`, 'is-recurring')
      + forecastScenarioCard('Incl. current-month recurring', recFc.forecastInclCurrent || recFc.forecastCentral, 'Reality lies between headline and recurring depending on lumpy items', 'is-recurring-alt')
      + `</div></div>`
    : '';
  const projection = forecast ? `<div class="rt-hf-projection"><div class="rt-headline-forecast-divider"><span class="rt-forecast-pill">PROJECTION</span> Full-year forecast</div>`
    ...
    + gapCallout + scenarioGrid + recBlock + bandGrid + formula + `</div>` : '';
...
export function renderMarketTable(monthlyAll, keys) {
  ...
  return `<table class="rt-table rt-striped">${head}<tbody>${rows}</tbody>${foot}</table>`
    + `<div class="rt-basis-note">${basisBadge('headline_gross')} All columns are 12-category gross from Booking Sheet (JLR-inclusive headline).</div>`;
}
...
export function renderCategoryTable(catsAll, keys, sortMode = 'market') {
  ...
  head += ytdHead() + monthHeaderCells(revMonths, false) + '<th>Total</th></tr></thead>';
  const body = pivot.categories.map((cat) => {
    ...
    const monthCells = monthValueCells(revMonths, (m) => cellVal(pivot, cat, m), false);
  ...
  const footCells = gtLabel() + gtEmpty(2) + `<td class="rt-year-col rt-grand-total-num">${fmtMoney(ytdTotal)}</td>`
    + monthFootMoneyCells(revMonths, monthTotals, false) + gtMoney(grandTotal);
...
export function renderProductBreakdownTable(findings, options = {}) {
  ...
  const head = `<thead><tr><th>Product</th><th class="rt-cat-col">Revenue category</th><th>12mo</th>${ytdHead()}<th>2026 ann.</th><th>2025</th><th>2024</th><th>Δ 25→26</th><th>Δ 24→25</th><th title="Auto flags from canonical_products + revenue shape (hover each chip).">Flags</th><th>2025 #</th></tr></thead>`;
  ...
  return `<div class="rt-basis-note">${includeJlr ? basisBadge('jlr_incl') : basisBadge('nonjlr_net')} Product revenue${includeJlr ? ' — JLR included.' : ' — JLR excluded by default.'}</div><table class="rt-table rt-striped rt-breakdown-table">${head}<tbody>${body}</tbody>${foot}</table>`;
}
...
export function renderPageBreakdownTable(findings, options = {}) {
  ...
  if (tierFilter) rows = rows.filter((r) => r.tier_key === tierFilter);
  if (search) rows = rows.filter((r) => r.unit.toLowerCase().includes(search.toLowerCase()));
  const head = `<thead><tr><th>Landing page</th><th>12mo</th>${ytdHead()}<th>2026 ann.</th><th>2025</th><th>2024</th><th>Δ 25→26</th><th>Δ 24→25</th><th>Badges</th><th>2025 #</th></tr></thead>`;
  const body = rows.map((r) => {
...
export function renderTierChartTable(monthlyAll, keys) {
  ...
  return `<div class="rt-basis-note">${basisBadge('headline_gross')} Headline gross · ${basisBadge('recurring_baseline')} recurring baseline (non-JLR minus lumpy).</div>`
    + `<table class="rt-table rt-striped"><thead><tr><th>Month</th><th>Headline £</th><th>Recurring £</th><th>Band</th><th></th></tr></thead><tbody>${rows}</tbody>${foot}</table>`;
}
