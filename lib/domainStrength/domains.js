  ...
  for (let i = 0; i < unique.length; i += chunkSize) {
    ...
    try {
      ...
      const checkUrl =
        ...
        `&search_engine=eq.${encodeURIComponent(engine)}` +
...
export async function dequeuePending({ engine = 'google', limit = 100 } = {}) {
  if (!supabaseUrl || !supabaseKey) {
    console.warn('[dequeuePending] Supabase not configured');
    return [];
  }
  ...
  try {
    ...
    console.log(`[dequeuePending] Fetching up to ${limit} domains from queue (engine: ${engine})`);
    ...
    if (!resp.ok) {
      ...
      console.error(`[dequeuePending] Query failed: ${resp.status} - ${errorText}`);
      return [];
    }
    const rows = await resp.json();
    const domains = Array.isArray(rows) ? rows.map(r => r.domain).filter(Boolean) : [];
    console.log(`[dequeuePending] Fetched ${domains.length} domains from queue`);
    return domains;
  } catch (e) {
    console.error('[dequeuePending] Error:', e.message);
    return [];
  }
}
