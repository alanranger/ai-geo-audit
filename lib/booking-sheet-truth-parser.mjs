...
export function parseBookingSheetTruth(workbook, options = {}) {
  ...
  if (monthlyPerCategory.length === 0 && transactionRows.length > 0) {
    monthlyPerCategory.push(...deriveMonthlyFromTransactions(transactionRows, propertyUrl));
    warnings.push(
      'Category grid had empty formula cells (workbook not recalculated in Excel). '
      + 'Monthly revenue rebuilt from transaction rows. '
      + 'For native grid import: open in Excel, press Ctrl+Alt+F9, Save, then re-upload.'
    );
    refreshVerificationFromCategories(verification, monthlyPerCategory, transactionRows);
  }
...
function deriveMonthlyFromTransactions(transactionRows, propertyUrl) {
  const buckets = new Map();
  const workbookByYear = new Map();
  for (const t of transactionRows) {
    const date = String(t.txn_date || '');
    if (date.length < 7) continue;
    const year = Number(date.slice(0, 4));
    const month = Number(date.slice(5, 7));
    const label = normalizeCategoryLabel(t.category_label);
    const order = t.category_order || parseCategoryOrder(label);
    if (!year || !month || !label || !order) continue;
    const key = `${year}\0${month}\0${order}\0${label}`;
    buckets.set(key, round2((buckets.get(key) || 0) + Number(t.amount || 0)));
    if (t.source_workbook) workbookByYear.set(year, t.source_workbook);
  }
  const out = [];
  for (const [key, revenue_amount] of buckets) {
    if (!revenue_amount) continue;
    const [ys, ms, os, category_label] = key.split('\0');
    out.push({
      property_url: propertyUrl,
      source_workbook: workbookByYear.get(Number(ys)) || `Sales ${ys}`,
      year: Number(ys),
      month: Number(ms),
      category_order: Number(os),
      category_label,
      revenue_amount,
      source_cell_range: 'derived:txns'
    });
  }
  return out.sort((a, b) => (a.year - b.year) || (a.month - b.month) || (a.category_order - b.category_order));
}

function refreshVerificationFromCategories(verification, monthlyPerCategory, transactionRows) {
  const byYear = new Map();
  for (const r of monthlyPerCategory) {
    let cell = byYear.get(r.year);
    if (!cell) {
      cell = { monthly: new Array(12).fill(0), sum: 0 };
      byYear.set(r.year, cell);
    }
    cell.monthly[r.month - 1] = round2(cell.monthly[r.month - 1] + Number(r.revenue_amount || 0));
    cell.sum = round2(cell.sum + Number(r.revenue_amount || 0));
  }
  const txnSumByYear = new Map();
  for (const t of transactionRows) {
    const y = Number(String(t.txn_date || '').slice(0, 4));
    if (!y) continue;
    txnSumByYear.set(y, round2((txnSumByYear.get(y) || 0) + Number(t.amount || 0)));
  }
  for (const v of verification) {
    const derived = byYear.get(v.year);
    const derivedYearSum = derived?.sum ?? 0;
    const derivedMonthly = derived?.monthly ?? new Array(12).fill(0);
    const ytd = v.ytdActualValue > 0 ? v.ytdActualValue : (txnSumByYear.get(v.year) ?? derivedYearSum);
    v.derivedFromCategories = derivedMonthly;
    v.derivedYearSum = derivedYearSum;
    v.monthlyTotalsRowSum = derivedYearSum;
    v.ytdActualValue = ytd;
    v.reconciles = Math.abs(derivedYearSum - ytd) < 0.01;
  }
}
