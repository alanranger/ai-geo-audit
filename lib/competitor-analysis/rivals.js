...
export function passesNoiseFilter(domainType, isCompetitor, independentsOnly, domainTypeSource) {
  ...
  return !NOISE_DOMAIN_TYPES.has(type) && type === 'site';
}

/** Consistent rivals: 2+ distinct money keywords on organic/local_pack. */
export function censusConsistentRivals(rows, targetRoot = 'alanranger.com', minMoneyKw = 2) {
  ...
  for (const row of rows || []) {
    ...
    for (const r of extracted.rivals) {
      if (!r.domain) continue;
      if (!byDomain.has(r.domain)) {
        byDomain.set(r.domain, { domain: r.domain, keywords: new Set(), hasLocalPack: false });
      }
      const rec = byDomain.get(r.domain);
      rec.keywords.add(kw);
      if (r.surface === 'local_pack') rec.hasLocalPack = true;
    }
  }

  return Array.from(byDomain.values())
    .filter((r) => r.keywords.size >= minMoneyKw)
    .map((r) => ({
      domain: r.domain,
      moneyKwCount: r.keywords.size,
      hasLocalPack: r.hasLocalPack,
    }))
    .sort((a, b) => b.moneyKwCount - a.moneyKwCount || a.domain.localeCompare(b.domain));
}

/** DFS/on-page spend gate: is_competitor + local presence + 2+ money kw. */
export function filterDfsGatedTargets(consistentRivals, metaByDomain) {
  return consistentRivals.filter((r) => {
    const m = metaByDomain[r.domain] || {};
    if (m.is_competitor !== true) return false;
    if (!r.hasLocalPack) return false;
    return r.moneyKwCount >= 2;
  });
}
...
export function topRivalsForKeyword(extracted, metaByDomain, independentsOnly, n = 3) {
  const filtered = (extracted?.rivals || []).filter((r) => {
    ...
    return passesNoiseFilter(
      m.domain_type, m.is_competitor === true, independentsOnly, m.domain_type_source
    );
  });
