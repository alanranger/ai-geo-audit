...
import { fmtMoney, DEFAULT_TIER_BANDS } from './revenue-truth-ui-core.mjs';
...
const BAND_CHIP = {
  below_survival: { text: 'Below survival', cls: 'rt-band-below_survival' },
...
function addRecurringBaselineWorry(candidates, summary) {
  ...
  const survival = rb?.survivalLine || summary?.config?.tierBands?.survival || DEFAULT_TIER_BANDS.survival;
  const avg = rb?.closedYtdRecurringAvg ?? rb?.janAprRecurringAvg;
  if (!avg || avg >= survival) return;
  const months = rb?.closedYtdCount || 0;
  const window = months ? `${months}-mo YTD` : 'YTD';
  pushWorry(candidates, {
    text: `Recurring baseline ${fmtMoney(avg, 0)}/mo (${window}) — below ${fmtMoney(survival, 0)} survival`,
...
function addHeadlineTrends(b, summary) {
  ...
  const cfg = summary?.config?.tierBands || DEFAULT_TIER_BANDS;
  const survivalLbl = fmtMoney(cfg.survival, 0);
  const comfortableLbl = fmtMoney(cfg.comfortable, 0);
  if (avg <= 0) return;
  if (avg < cfg.survival) {
    b.trends.push({
      text: `Trailing 3-mo avg ${fmtMoney(avg, 0)} — ${fmtMoney(cfg.survival - avg, 0)} below ${survivalLbl} survival`,
      sub: `Primary monthly target is ${survivalLbl} survival`
    });
  } else {
    b.trends.push({
      text: `Trailing 3-mo avg ${fmtMoney(avg, 0)} — above ${survivalLbl} survival`,
      sub: avg < cfg.comfortable ? `Still ${fmtMoney(cfg.comfortable - avg, 0)} below ${comfortableLbl} comfortable` : 'At or above comfortable band'
    });
  }
}
...
function addForecastTrends(b, summary, findings) {
  ...
  if (central < YEAR_SURVIVAL) {
    b.trends.push({
      text: `Full-year projected ${fmtMoney(central, 0)} — below ${fmtMoney(YEAR_SURVIVAL, 0)} year survival`,
      sub: basis
    });
    return;
  }
...
function addWorkingBullets(b, findings, summary) {
  ...
  if (central >= YEAR_SURVIVAL && central < YEAR_COMFORTABLE) {
    b.working.push({
      text: `Projected ${fmtMoney(central, 0)} reaches above ${fmtMoney(YEAR_SURVIVAL, 0)} year survival`,
...
function addInvestigateBullets(b, diagnosis, windowMonths) {
  ...
  for (const d of diags) {
    ...
    const isHub = isHubInvestigateSlug(d.page_slug);
    b.investigate.push({
      text: `${d.page_slug}: ${fmtN(fw.impressions)} impressions, ${fmtN(fw.clicks)} clicks, £0 mapped revenue (${windowMonths}mo)`,
      sub: isHub
        ? 'Hub page — routes visitors to product pages (no direct conversion)'
        : 'Traffic with zero conversion',
      isHub
    });
  }
}
...
function buildMonthlyTracker(summary, findings) {
  ...
  const bands = summary?.config?.tierBands || DEFAULT_TIER_BANDS;
...
function yearGapText(gap) {
  const yr = fmtMoney(YEAR_SURVIVAL, 0);
  if (gap < 0) return `▼ ${fmtMoney(Math.abs(gap), 0)} below ${yr} yr survival`;
  if (gap > 0) return `▲ ${fmtMoney(gap, 0)} above ${yr} yr survival`;
  return `On ${yr} year survival line`;
}

function yearForecastChip(value) {
  const yr = fmtMoney(YEAR_SURVIVAL, 0);
  if (value < YEAR_SURVIVAL) return { text: `Below ${yr} yr`, cls: 'rt-band-below_survival' };
  if (value < YEAR_COMFORTABLE) return { text: `Above ${yr} yr`, cls: 'rt-band-survival' };
...
function bandChip(band, bands) {
  const chip = BAND_CHIP[band] || BAND_CHIP.below_survival;
  if (chip === BAND_CHIP.below_survival && bands?.survival) {
    return { ...chip, text: `Below ${fmtMoney(bands.survival, 0)}` };
  }
  return chip;
}

function survivalLine(bands) {
  return Number(bands?.survival) || DEFAULT_TIER_BANDS.survival;
}

function gapText(gap, suffix = '', survival = DEFAULT_TIER_BANDS.survival) {
  const sfx = suffix ? ` ${suffix}` : '';
  const lbl = fmtMoney(survival, 0);
  if (gap < 0) return `▼ ${fmtMoney(Math.abs(gap), 0)} below ${lbl}${sfx}`;
  if (gap > 0) return `▲ ${fmtMoney(gap, 0)} above ${lbl}${sfx}`;
  return `On ${lbl} line`;
}
...
function closedMonthRow(m, bands) {
  ...
  return {
    ...
    gapText: gapText(gap, '', survival),
    gapDir: gapDir(gap),
    chip: bandChip(classifyBand(actual, bands), bands),
...
function currentMonthRow(m, pulse, bands) {
  ...
  if (defcon?.active) {
    ...
    return {
      ...
      gapText: gapText(gap, '(proj.)', survival),
      gapDir: gapDir(gap),
  ...
  return {
    ...
    gapText: gapText(gap, '(pace)', survival),
    gapDir: gapDir(gap),
    chip: bandChip(classifyBand(pace, bands), bands),
...
function renderBullet(it) {
  const hubBadge = it.isHub ? '<span class="rt-exec-hub-badge" title="Hub page">🔗 hub</span> ' : '';
  ...
  return `<li>${hubBadge}${main}${sub}</li>`;
}
