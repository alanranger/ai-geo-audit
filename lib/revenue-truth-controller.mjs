...
import { headlineSignals, forecastSignals, channelSignals, clientsSignals } from './revenue-truth-key-signals.mjs';
...
const rtDiagExpandedTiers = new Set(['workshops_non_residential', 'courses_masterclasses']);
...
function applyHeadlineStrip() {
  const sig = document.getElementById('rt-headline-signals');
  if (sig && rtData) { sig.style.display = 'block'; sig.innerHTML = headlineSignals(rtData.headlineStrip, rtData.config); }
  const stripEl = document.getElementById('rt-strip');
  if (stripEl && rtData?.headlineStrip) stripEl.innerHTML = renderHeadlineStripHtml(rtData.headlineStrip, rtData.config);
}
...
function renderAllSections() {
  ...
  applyHeadlineStrip();
  renderTierChart();
  const fcBody = document.getElementById('rt-forecast-body');
  if (fcBody && rtData) fcBody.innerHTML = renderForecastHtml(rtData.forecast);
  const fcSig = document.getElementById('rt-forecast-signals');
  if (fcSig && rtData) { fcSig.style.display = 'block'; fcSig.innerHTML = forecastSignals(rtData.forecast); }
