...
function renderTierChart() {
  ...
  if (leg) {
    leg.innerHTML = ['thrive', 'comfortable', 'survival', 'below_survival'].map((b) =>
      ...
      + `<span class="rt-basis-badge" data-basis="headline_gross">Headline (12-category gross)</span>`;
  }
  const sub = document.getElementById('rt-section-1-sub');
  if (sub) sub.textContent = 'Bars use headline gross (12-category Booking Sheet, JLR-inclusive). Pulse + tier reconciliation use non-JLR net.';
}
...
function applyHeadlineForecastPanel() {
  ...
  if (body) body.innerHTML = renderHeadlineForecastPanelHtml(rtData.headlineStrip, rtData.config, rtData.forecast, rtData.currentMonthPulse);
}
