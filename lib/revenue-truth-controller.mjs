...
function renderTierChart() {
  const host = document.getElementById('rt-tier-chart-host') || document.querySelector('#rt-tier-chart-section .rt-tier-chart-wrap');
  if (!host || !rtData) return;
  const keys = monthKeys();
  const monthly = rtData.monthly.filter((m) => keys.has(`${m.year}|${m.month}`));
  host.innerHTML = renderTierChartSvg(monthly, rtData.config, rtIncludeJlr);
  ...
  if (leg) {
    leg.innerHTML = ['thrive', 'comfortable', 'survival', 'below_survival'].map((b) =>
      ...
      + basisNoteHtml()
      + `<span class="rt-basis-badge" data-basis="recurring_baseline" title="Recurring baseline = headline minus residential workshops, voucher tiers, and event-bound products.">Recurring baseline (hatched)</span>`;
  }
  const sub = document.getElementById('rt-section-1-sub');
  if (sub) {
    sub.textContent = rtIncludeJlr
      ? 'Inline bars with £ labels: headline gross (solid) vs recurring baseline (hatched). Reference lines at £3k / £5k / £8k survival bands.'
