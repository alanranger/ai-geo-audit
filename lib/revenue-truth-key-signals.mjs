...
import { fmtMoney, fmtN, DEFAULT_TIER_BANDS, TIER_BAND_TARGETS } from './revenue-truth-ui-core.mjs';
...
export function tierCardSignals(t, includeJlr = false) {
  const bullets = [];
  ...
  const revKey = includeJlr ? 'total' : 'non_jlr';
  const y25 = rt.y2025?.[revKey] || 0;
  const y26 = rt.y2026_ytd?.[revKey] || 0;
...
export function headlineSignals(strip, cfg) {
  ...
  const survival = cfg?.tierBands?.survival || DEFAULT_TIER_BANDS.survival;
...
export function forecastSignals(forecast) {
  ...
  if (central < TIER_BAND_TARGETS.survival) bullets.push({ text: `⚠ Forecast ${fmtMoney(central, 0)} is below £53.4k survival.`, cls: 'is-warn' });
...
export function headlineForecastSignals(strip, cfg, forecast, pulse) {
  const items = [];
  if (strip) {
    const band = strip.trailing3Band;
    const avg = strip.trailing3MonthAverage || 0;
    ...
    const comfortable = cfg?.tierBands?.comfortable || 5000;
    if (band === 'below_survival' || avg < survival) {
      items.push({ text: `⚠ Trailing 3-month average ${fmtMoney(avg, 0)} — below survival band (£${survival.toLocaleString('en-GB')}/mo).`, cls: 'is-warn' });
    } else if (band === 'survival' || avg < comfortable) {
      items.push({ text: `⚠ Trailing 3-month average ${fmtMoney(avg, 0)} — survival band, below comfortable (£${comfortable.toLocaleString('en-GB')}/mo).`, cls: 'is-warn' });
    } else if (band === 'comfortable' || band === 'thrive') {
      items.push({ text: `✓ Trailing 3-month average ${fmtMoney(avg, 0)} — ${band === 'thrive' ? 'thrive' : 'comfortable'} band.`, cls: 'is-good' });
    }
    const ytd = strip.ytd;
    if (ytd && ytd.ytdRevenue < ytd.proRataTarget * 0.95) {
      items.push({ text: `⚠ YTD headline ${fmtMoney(ytd.ytdRevenue, 0)} is below pro-rata target ${fmtMoney(ytd.proRataTarget, 0)}.`, cls: 'is-warn' });
    }
  }
  const fi = pulse?.forecast_impact;
  const closed = forecast?.forecastCentral || 0;
  const live = fi?.revised_forecast_primary ?? forecast?.forecastInclCurrent;
  if (live != null && Math.abs(live - closed) >= 500) {
    items.push({
      text: `⚠ Closed-month-only ${fmtMoney(closed, 0)} ignores live month — revised ${fmtMoney(live, 0)} (${fmtMoney(live - closed, 0)}).`,
      cls: 'is-warn'
    });
  } else if (forecast) {
    const central = forecast.forecastCentral || 0;
    if (central < TIER_BAND_TARGETS.survival) items.push({ text: `⚠ Forecast ${fmtMoney(central, 0)} is below £53.4k survival.`, cls: 'is-warn' });
    else if (central < 60000) items.push({ text: `⚠ Forecast ${fmtMoney(central, 0)} is below £60k comfortable (${fmtMoney(60000 - central, 0)} gap).`, cls: 'is-warn' });
    else items.push({ text: `✓ Forecast ${fmtMoney(central, 0)} reaches the £60k comfortable target.`, cls: 'is-good' });
  }
  return bulletsHtml(items);
}
