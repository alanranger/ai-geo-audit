/**
 * Recurring baseline = countable net minus voucher tiers (gift vouchers + Pick-n-Mix).
 ...
 *
 * Residential workshops and seasonal event products (Bluebell, Heather, Lavender,
 * etc.) are treated as part of the year-round baseline and are NOT stripped — they
 * recur every year and are core business products. Only voucher tiers are removed
...
// seasonalityByProduct kept for signature stability; event-bound products are now
// part of the baseline so seasonality no longer affects the lumpy decision.
export function isLumpyTxn(t, seasonalityByProduct, includeJlr = false) {
  if (!isCountableTxn(t, includeJlr)) return false;
  return RECURRING_LUMPY_TIER_KEYS.has(tierFromBookingCategory(t.category_label));
}
