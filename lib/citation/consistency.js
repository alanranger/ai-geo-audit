...
const DIRECTORY_DOMAIN_ALIASES = {
  'rustpilot.com': 'trustpilot.com'
};
...
const normalizeDirectoryDomain = (value) => {
  const normalized = normalizeText(value);
  if (!normalized) return '';
  return DIRECTORY_DOMAIN_ALIASES[normalized] || normalized;
};
...
const parseDomainList = (raw) => {
  const configured = String(raw || '')
    .split(',')
    .map((value) => normalizeDirectoryDomain(value))
...
const extractHost = (value) => {
  try {
    return String(new URL(String(value || '').trim()).hostname || '').toLowerCase();
  } catch (err) {
    return '';
  }
};
...
const buildDiscoveryTerms = ({ canonicalNap, brandName, propertyUrl }) => {
  const propertyHost = extractHost(propertyUrl).replace(/^www\./, '');
  const name = normalizeText(canonicalNap?.name || brandName || '');
  const nameNoSuffix = name.replace(/\bphotography\b/g, '').replace(/\s+/g, ' ').trim();
...
const buildDomainQueries = (domain, discoveryTerms) => {
  const termQueries = discoveryTerms.map((term) => `site:${domain} "${term}"`);
  const combos = [];
  if (discoveryTerms.length >= 2) {
    combos.push(`site:${domain} "${discoveryTerms[0]}" "${discoveryTerms[1]}"`);
  }
  return unique([...combos, ...termQueries]).slice(0, 8);
};

const scoreCandidate = (candidate, discoveryTerms) => {
  const title = normalizeText(candidate.title || '');
  const snippet = normalizeText(candidate.snippet || '');
  const link = normalizeText(candidate.link || '');
  return discoveryTerms.reduce((sum, term) => {
    const value = normalizeText(term);
    if (!value) return sum;
    let next = sum;
    if (title.includes(value)) next += 3;
    if (snippet.includes(value)) next += 2;
    if (link.includes(value)) next += 3;
    return next;
  }, 0);
};
...
const fetchDomainCandidates = async ({ domain, discoveryTerms, perDomainLimit }) => {
  const queries = buildDomainQueries(domain, discoveryTerms);
  const collected = [];
  ...
  for (const query of queries) {
    ...
    const parsed = parseBingRss(xml).filter((item) => isUrlOnDomain(item.link, domain));
    collected.push(...parsed);
  }

  const uniqueCandidates = unique(
    collected.map((item) => JSON.stringify({ ...item, link: toPathlessUrl(item.link) }))
  ).map((value) => JSON.parse(value));

  return uniqueCandidates
    .sort((a, b) => scoreCandidate(b, discoveryTerms) - scoreCandidate(a, discoveryTerms))
...
export const collectCitationConsistencyRows = async ({
  ...
  const discoveryTerms = buildDiscoveryTerms({ canonicalNap, brandName, propertyUrl });
  ...
  const candidates = await runParallel(domains, concurrency, async (domain) => {
    ...
    const domainCandidates = await fetchDomainCandidates({ domain, discoveryTerms, perDomainLimit });
