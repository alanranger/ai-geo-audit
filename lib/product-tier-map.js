...
//     -> last write wins. Each row can carry any one of three keys:
//        - url_slug       : exact path match (for products with a stable URL).
//        - product_id     : SQ product GUID match (for SQ Commerce Orders
//                           line items, which return blank productUrl, so
//                           url_slug matching is impossible).
//        - title_prefix   : titlePrefix() match (for products without a
//                           stable URL or stable productId - e.g. custom
//                           commission lines, academy add-ons).
...
async function fetchOverrides(supabase) {
  const { data, error } = await supabase
    ...
    .select('url_slug, tier_id, product_id, title_prefix');
  if (error) return [];
  return data || [];
}
...
// Overrides win over csv_metadata for the same key. A row may set any one
// of url_slug / product_id / title_prefix (or several at once); we apply
// each non-empty key to its respective lookup table. The url_slug PK can
// be a synthetic value like `__title:foo` or `__pid:bar` for rows that
// don't carry a real path; only paths starting with `/` populate the
// slug map.
function ingestOverride(maps, row) {
  if (!row?.tier_id) return;
  const tier = row.tier_id;
  const slug = String(row.url_slug || '').toLowerCase();
  if (slug.startsWith('/')) maps.slugToTier.set(slug, tier);
  const pid = String(row.product_id || '').trim();
  if (pid) maps.productIdToTier.set(pid, tier);
  const prefix = String(row.title_prefix || '').toLowerCase().trim();
  if (prefix) maps.titleToTier.set(prefix, tier);
}
...
// Look up productUrl -> tier (deterministic), then productId -> tier
// (for SQ orders that come back without a productUrl), and finally
// titlePrefix(productName) -> tier (for products without a stable URL
// or productId). Returns null if no map entry matches; the caller then
// falls through to the legacy name/url-token rules in commercial-tier.js.
export function classifyByMap(map, productUrl, productName, productId) {
  if (!map) return null;
  ...
  if (pid && map.productIdToTier?.has(pid)) {
    return map.productIdToTier.get(pid);
  }
