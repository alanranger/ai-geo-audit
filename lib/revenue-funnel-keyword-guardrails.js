...
export function intentClassForUrl(url) {
  ...
  if (/hire-me|hire|professional-photographer/.test(p)) {
    return 'commercial_hire';
  }
  if (/photography-courses-coventry|photography-course-coventry/.test(p)) {
...
import { pageHeadNounFromUrl } from './revenue-funnel-serp-copy.js';
...
export function safeTitleLead(url, keyword) {
  ...
  if (intent === 'local_courses' || noun === 'courses') {
    if (/\bonline\b/i.test(kw)) {
    ...
    return {
      ...
      note: 'URL/tier own "courses + Coventry". Ranking keyword may say "lessons" — keep page noun; tune title/meta only.'
    };
  }
  if (intent === 'local_workshops' || noun === 'workshops') {
    return {
      lead: /workshop/i.test(kw) ? kw : 'Photography Workshops',
      note: 'Workshops page — do not reuse Coventry courses hub title/meta.'
    };
  }
  if (intent === 'commercial_hire' || noun === 'hire') {
    return {
      lead: /hire|photographer in coventry|professional photographer/i.test(kw)
        ? kw
        : 'Hire a Professional Photographer in Coventry',
      note: 'Hire/commercial page — courses queries belong on /photography-courses-coventry.'
    };
  }
  return { lead: kw, note: null };
}
...
function applyOwnershipRules(list, keywords) {
  return list.map((c) => {
    ...
    } else if (intent === 'commercial_hire' && /\b(courses?|lessons?|workshops?)\b/i.test(kw)) {
      c.guardrail_severity = 'cannibalization';
      c.guardrail_blocked_top3 = true;
      c.guardrail_notes = [...notes, `"${kw}" is a courses/workshops query — use /photography-courses-coventry, not this hire page.`];
      c.weighted_score = (Number(c.weighted_score) || 0) * 0.1;
