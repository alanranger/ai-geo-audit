...
export async function runCeoWeeklyReport(opts = {}) {
  ...
  async function logRefreshRun(extra = {}) {
    const log = opts.refreshLog;
    if (!log || typeof log !== 'object') return null;
    const row = {
      week_start: weekStart,
      status: log.status || extra.status || 'ok',
      steps: Array.isArray(log.steps) ? log.steps : [{ key: 'manual_full_refresh', ok: true }],
      finished_at: log.finished_at || new Date().toISOString(),
      error: log.error || extra.error || null,
      started_at: log.started_at || new Date().toISOString()
    };
    // Table keeps a history of runs (multiple rows per week_start) — always insert
    const { data, error } = await supabase
      .from('ceo_weekly_refresh_runs')
      .insert(row)
      .select('id, week_start, status, finished_at')
      .maybeSingle();
    if (error) throw new Error(`refresh_run_log:${error.message}`);
    return data;
  }

  // Avoid double-send on the Monday backup cron only — Full Refresh always re-sends
  if (!forceResend && !dryRun) {
    const { data: prior } = await supabase
      ...
      .maybeSingle();
    if (prior?.send_status === 'sent' && prior?.sent_at) {
      return {
        mode: 'already_sent',
        weekStart,
        numbers_included: true,
        send: { ok: true, skipped: true, reason: 'already_sent_this_week', sent_at: prior.sent_at }
      };
    }
  }
  ...
  if (!gate.ok) {
    ...
    const refreshRun = await logRefreshRun({ status: 'partial', error: gate.reason });
    const { data: snap } = await supabase
      ...
      .upsert({
        ...
        html_preview: html,
        refresh_run_id: refreshRun?.id || gate.run?.id || null,
  ...
  const send = await sendCeoWeeklyEmail({ subject, html, to: CEO_REPORT_TO, dryRun });
  const refreshRun = await logRefreshRun({ status: 'ok' });
