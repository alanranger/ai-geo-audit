...
function moverLines(items, color) {
  if (!items?.length) return `<span style="color:${GREY}">none this period</span>`;
  return items.map((m) => {
...
function renderDefSections(metrics) {
  ...
  const rkKwWow = sh.ranking_keywords_wow;
  let rkKwLabel = '';
  if (rkKwWow?.delta != null) {
    const d = Number(rkKwWow.delta);
    rkKwLabel = ` ${d > 0 ? '▲' : d < 0 ? '▼' : '→'}${fmtNum(Math.abs(d))}`;
  } else {
    rkKwLabel = ' → n/a';
  }
  ...
  const avgPosWow = rk.wow?.avg_position;
  const moneyAvgWow = rk.wow?.money_avg_position;
  const avgPosDelta = (avgPosWow?.pending || avgPosWow?.delta == null)
    ? 'from next Monday'
    : (avgPosWow.label || '→ 0');
  const moneyAvgDelta = (moneyAvgWow?.pending || moneyAvgWow?.delta == null)
    ? 'from next Monday'
    : (moneyAvgWow.label || '→ 0');
  const bucketCell = (key, bg, color, label) => {
    const n = buckets[key];
    const wow = rk.wow?.[key];
    let deltaBit = '';
    if (!rk.buckets_pending && wow && !wow.pending && wow.delta != null) {
      const d = Number(wow.delta);
      deltaBit = d === 0
        ? ` <span style="font-size:10px;color:${GREY};font-weight:600;">→0</span>`
        : ` <span style="font-size:10px;color:${d > 0 ? GREEN : RED};font-weight:600;">${d > 0 ? '▲' : '▼'}${Math.abs(Math.round(d))}</span>`;
    }
    return `<td style="background:${bg};border-radius:8px;padding:8px;"><div style="font-size:18px;font-weight:800;color:${color};${FF}">${esc(fmtNum(n))}${deltaBit}</div><div style="font-size:10px;color:#667085;${FF}">${label}</div></td>`;
  };
  ...
  <tr><td style="padding:8px 28px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:13px;${FF}">
      ...
      ${row3('Spam score', `${esc(fmtNum(bl.spam_score))}${bl.spam_label ? ` · ${esc(bl.spam_label)}` : ''}`, snapDeltaHtml(bl.wow?.spam_score))}
  ...
  <tr><td style="padding:8px 28px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="text-align:center;">
      <tr>
        ${bucketCell('b1', '#f0faf4', GREEN, '#1')}
        <td style="width:6px;"></td>
        ${bucketCell('b2_3', '#f0faf4', GREEN, '#2–3')}
        <td style="width:6px;"></td>
        ${bucketCell('b4_10', '#f7f9fc', '#101828', '#4–10')}
        <td style="width:6px;"></td>
        ${bucketCell('b11_20', '#fdf4e3', AMBER, '#11–20')}
        <td style="width:6px;"></td>
        ${bucketCell('b21', '#fdf1f0', RED, '#21+')}
      </tr>
    </table>
    ${rk.buckets_pending ? `<div style="color:${GREY};font-size:11px;padding:6px 0 0;${FF}">Δ from next Monday</div>` : ''}
  </td></tr>
  <tr><td style="padding:12px 28px 0;">
    ...
    <div style="color:${GREY};font-size:11px;padding:8px 0 0;${FF}">Avg position (tracked) ${esc(avgPos)} ${esc(avgPosDelta)} · money-page avg ${esc(moneyAvg)} ${esc(moneyAvgDelta)} · window ~28d.</div>
  </td></tr>
  ...
  <tr><td style="padding:8px 28px 24px;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td style="width:8px;"></td>
        <td style="width:33%;background:#f7f9fc;border-radius:8px;padding:10px;text-align:center;vertical-align:top;"><div style="font-size:22px;font-weight:800;color:#101828;${FF}">${esc(fmtNum(sh.ranking_keywords))}</div><div style="font-size:10px;color:#667085;${FF}">ranking keywords${esc(rkKwLabel)}</div></td>
      </tr>
    </table>
...
export function renderCeoWeeklyHtml(metrics) {
  ...
  const pulse = metrics.current_month_pulse || {};
  const lines = metrics.revenue_by_line?.lines || [];
  const salesCategoryRows = lines.map((l) => {
    const label = l.short_label || String(l.category_label || '').replace(/^\d+\.?\s*/, '');
    return row3(label, esc(fmtGbp(l.amount)), deltaHtml(l.wow, { money: true }));
  }).join('');
  const pulseDelta = pulse.miss_vs_survival != null
    ? `<span style="color:${RED};font-weight:600">${esc(`▼ £${Math.round(Number(pulse.miss_vs_survival)).toLocaleString('en-GB')} vs survival`)}</span>`
    : `<span style="color:${GREY};font-weight:600">→ n/a</span>`;
  const pulseLabel = pulse.month_label
    ? `${pulse.month_label} projected (Current Month Pulse)`
    : 'Current month projected (Revenue Truth)';
  const pulseValue = pulse.projected_month_end != null
    ? esc(fmtGbp(pulse.projected_month_end))
    : '—';
  const pulseNote = (pulse.month_label && pulse.days_remaining != null)
    ? `Current Month Pulse: ${pulse.month_label} · ${pulse.days_remaining} days left · DEFCON ${pulse.defcon_level || '—'} ${pulse.defcon_status || ''}`.trim()
    : null;
  ...
  const rollEndInclusive = roll.end
    ? (() => {
      const t = Date.parse(`${roll.end}T12:00:00Z`) - 86400000;
      return new Date(t).toISOString().slice(0, 10);
    })()
    : null;
  const fmtRollDay = (ymd) => (ymd
    ? new Date(`${ymd}T12:00:00Z`).toLocaleDateString('en-GB', {
      day: 'numeric', month: 'short', year: 'numeric', timeZone: 'UTC'
    })
    : null);
  const rollFrom = fmtRollDay(roll.start);
  const rollTo = fmtRollDay(rollEndInclusive || roll.end);
  const rollPeriodNote = (rollFrom && rollTo)
    ? `Figures are rolling 28 days from ${rollFrom} to ${rollTo} (inclusive), compared with the prior 28 days. Not calendar month / MTD.`
    : 'Figures are rolling 28 days vs the prior 28 days. Not calendar month / MTD.';
  const rollLabel = 'Sales (28d) vs £4,450 survival';
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" align="center" style="width:600px;max-width:600px;background:#ffffff;border-radius:14px;overflow:hidden;">
  ...
  <tr><td style="padding:8px 28px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:13px;${FF}">
      ...
      ${row3('Keywords on page 1', `${esc(fmtNum(rk.page1))} / ${esc(fmtNum(rk.tracked))}`, snapDeltaHtml(rk.wow?.page1))}
      ${row3Last('Authority (E-E-A-T)', esc(fmtNum(auth.score)), auth?.stale
        ? `<span style="color:${GREY};font-weight:600">→ n/a</span>`
        : snapDeltaHtml(auth?.wow?.delta == null && !auth?.wow?.pending
          ? { pending: true, label: 'from next Monday', delta: null }
          : (auth?.wow || { pending: true, label: 'from next Monday', delta: null })))}
    </table>
  ...
  <tr><td style="padding:8px 28px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:13px;${FF}">
      ${row3(rollLabel, esc(fmtGbp(roll.amount)), deltaHtml(roll.wow, { money: true }))}
      ${row3(pulseLabel, pulseValue, pulseDelta)}
      ${salesCategoryRows}
    ...
    <div style="color:#344054;font-size:11.5px;padding:8px 0 2px;${FF}"><b>Period:</b> ${esc(rollPeriodNote)}</div>
    ${pulseNote ? `<div style="color:#344054;font-size:11.5px;padding:2px 0;${FF}"><b>Projection:</b> ${esc(pulseNote)} — same as Revenue Truth tile.</div>` : ''}
    <div style="color:#667085;font-size:11.5px;padding:4px 0 6px;${FF}">Sales categories (excl. Pick n Mix + Gift Vouchers Out) · Top pages to fix (upside clicks 28d): ${upsideLine}.</div>
  </td></tr>
