...
export function renderCeoWeeklyHtml(metrics) {
  ...
  const rollFrom = roll.start
    ? new Date(`${roll.start}T12:00:00Z`).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'UTC' })
    : null;
  const rollTo = roll.end
    ? new Date(`${roll.end}T12:00:00Z`).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'UTC' })
    : null;
  const rollPeriodNote = (rollFrom && rollTo)
    ? `Figures are rolling 28 days (${rollFrom} → ${rollTo}), vs the prior 28 days. Not calendar month / MTD.`
