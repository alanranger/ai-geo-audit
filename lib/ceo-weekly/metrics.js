...
async function sectionBacklinks(supabase, prior) {
  ...
  return {
    rank: num(cache?.rank),
    backlinks: num(cache?.backlinks) ?? tiles?.totalBacklinks ?? null,
    referring_domains: num(cache?.referring_domains) ?? tiles?.referringDomains ?? null,
    spam_score: num(cache?.backlinks_spam_score),
    crawled_pages: num(cache?.crawled_pages),
    dofollow: tiles?.dofollow ?? null,
    nofollow: tiles?.nofollow ?? null,
    ...
    wow: {
      referring_domains: deltaArrow(
        num(cache?.referring_domains) ?? tiles?.referringDomains,
        priorBl.referring_domains
      ),
      backlinks: deltaArrow(num(cache?.backlinks) ?? tiles?.totalBacklinks, priorBl.backlinks),
      rank: deltaArrow(num(cache?.rank), priorBl.rank),
      dofollow: deltaArrow(tiles?.dofollow, priorBl.dofollow),
      nofollow: deltaArrow(tiles?.nofollow, priorBl.nofollow)
    }
  };
}
