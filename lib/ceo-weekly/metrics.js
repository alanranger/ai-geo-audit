...
async function sectionBacklinks(supabase, prior) {
  ...
  const priorBands = priorBl.da_bands || [];
  const da_bands = DA_BAND_ORDER.map(({ key, label }) => {
    ...
    return {
      ...
      dofollow,
      nofollow,
      wow_dofollow: deltaArrow(dofollow, priorRow?.dofollow),
      wow_nofollow: deltaArrow(nofollow, priorRow?.nofollow)
    };
  });
  ...
  return {
    ...
    wow: {
      referring_domains: deltaArrow(blRd, priorBl.referring_domains),
      backlinks: deltaArrow(blTotal, priorBl.backlinks),
      rank: deltaArrow(num(cache?.rank), priorBl.rank),
      dofollow: deltaArrow(blDf, priorBl.dofollow),
      nofollow: deltaArrow(blNf, priorBl.nofollow)
    }
  };
}
...
function buildNarrative(metrics) {
  ...
  const biggestDrop = [...lines]
    .filter((l) => l.wow?.delta != null)
    .sort((a, b) => (a.wow.delta || 0) - (b.wow.delta || 0))[0];
  ...
  if (topLine?.wow?.delta > 0) {
    good.push(`${topLine.category_label.replace(/^\d+\.?\s*/, '')} ${topLine.wow.label}`);
    wins.push(`${topLine.category_label.replace(/^\d+\.?\s*/, '')} ${fmtGbp(topLine.wow.delta)}`);
  }
  ...
  if (biggestDrop?.wow?.delta < 0) {
    worries.push(`${biggestDrop.category_label.replace(/^\d+\.?\s*/, '')} ${fmtGbp(biggestDrop.wow.delta)}`);
  }
  ...
  const moneyVerdict = {
    ...
    line: `${fmtGbp(cm.amount)}, <b style="color:#b42318;">${fmtGbp(Math.abs(rt.gap_vs_survival || 0))} under survival</b>. Down <b style="color:#b42318;">${cm.wow?.label || ''}</b>${biggestDrop?.wow?.delta < 0 ? ` — ${biggestDrop.category_label.replace(/^\d+\.?\s*/, '')} <b style="color:#b42318;">${fmtGbp(biggestDrop.wow.delta)}</b>` : ''}.${one21?.wow?.delta > 0 ? ` Only 1-2-1 grew <b style="color:#067647;">${one21.wow.label}</b>.` : ''}`
  };
