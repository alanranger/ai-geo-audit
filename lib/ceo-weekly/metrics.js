...
function buildNarrative(metrics) {
  ...
  const moneyWord = (roll.amount || 0) >= (rt.survival || 4450) ? 'HEALTHY' : 'DECLINE';
  const underGap = Math.abs(rt.gap_vs_survival || 0);
  const underBit = (roll.amount || 0) < (rt.survival || 4450)
...
export async function buildCeoWeeklyMetrics(supabase, opts = {}) {
  ...
  const metrics = {
    ...
    definitions: {
      ...
      survival: 'DEFAULT_TIER_BANDS.survival = 4450 (compared to rolling 28d sales)',
