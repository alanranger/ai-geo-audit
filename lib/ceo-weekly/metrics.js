 ...
 * Money headline = rolling 28d (vs prior 28d) — weekly CEO brief must show trend,
 * not last completed calendar month.
 */
...
// Pick n Mix Inc/Out + Gift Vouchers Out intentionally omitted from CEO email.
...
async function txnSumNonJlr(supabase, startYmd, endYmdExclusive) {
  const { data, error } = await supabase
    ...
    .select('amount, is_jlr, txn_date, category_label')
  ...
  for (const t of data || []) {
    ...
    sum += amount;
    count += 1;
    const label = String(t.category_label || '').trim();
    if (label) byLabel.set(label, (byLabel.get(label) || 0) + amount);
  }
  return { sum, count, byLabel };
}
...
async function sectionRevenueTruth(supabase, prior) {
  ...
  const curMonth = await monthCategoryTotal(supabase, completed.year, completed.month);
  const prevMonth = await monthCategoryTotal(supabase, priorMonth.year, priorMonth.month);
  ...
  const defcon = computeDefcon(roll.sum, survival);
  void prior;

  return {
    headline_mode: 'rolling_28d',
    completed_month: {
      year: completed.year,
      month: completed.month,
      label: monthLabel(completed.year, completed.month),
      amount: curMonth.total,
      wow: moneyDeltaArrow(curMonth.total, prevMonth.total)
    },
    rolling_4wk: {
      amount: roll.sum,
      txn_count: roll.count,
      start: rollStart,
      end: today,
      wow: moneyDeltaArrow(roll.sum, priorRoll.sum)
    },
    survival,
    gap_vs_survival: roll.sum - survival,
    ...
    under_survival: roll.sum < survival,
    mtd: roll.sum,
    wow: moneyDeltaArrow(roll.sum, priorRoll.sum),
    source: 'booking_sheet_transactions non-JLR rolling 28d vs prior 28d'
  };
}

async function sectionRevenueByLine(supabase) {
  const today = londonYmd();
  const rollStart = ymdAdd(today, -28);
  const priorRollStart = ymdAdd(today, -56);
  const cur = await txnSumNonJlr(supabase, rollStart, today);
  const prev = await txnSumNonJlr(supabase, priorRollStart, rollStart);
  ...
  return {
    ...
    biggest_rise,
    total,
    wow: moneyDeltaArrow(total, prev.sum),
    source: 'booking_sheet_transactions by category — rolling 28d vs prior 28d'
  };
}
...
async function rankingRowsForDate(supabase, propertyUrl, auditDate) {
  if (!auditDate) return [];
...
async function latestRankingRows(supabase, propertyUrl) {
  const { data: latestRows } = await supabase
    ...
    .limit(1);
  const auditDate = latestRows?.[0]?.audit_date || null;
  const rows = await rankingRowsForDate(supabase, propertyUrl, auditDate);
  return { auditDate, rows };
}

async function priorRankingAuditDate(supabase, propertyUrl, currentAuditDate) {
  if (!currentAuditDate) return null;
  const { data } = await supabase
    .from('keyword_rankings')
    .select('audit_date')
    .eq('property_url', propertyUrl)
    .lt('audit_date', currentAuditDate)
    .order('audit_date', { ascending: false })
    .limit(1);
  return data?.[0]?.audit_date || null;
}

function summariseRankingRows(rows) {
  ...
  return {
    buckets,
    page1: buckets.b1 + buckets.b2_3 + buckets.b4_10,
    tracked,
    avg_position: rankN ? rankSum / rankN : null,
    money_avg_position: moneyN ? moneySum / moneyN : null
  };
}
...
async function sectionRankings(supabase, propertyUrl, prior) {
  ...
  const cur = summariseRankingRows(rows);

  // Prefer prior keyword_rankings audit (real ranking history); fall back to sent CEO snapshot.
  const priorAuditDate = await priorRankingAuditDate(supabase, propertyUrl, auditDate);
  let priorSum = null;
  if (priorAuditDate) {
    priorSum = summariseRankingRows(await rankingRowsForDate(supabase, propertyUrl, priorAuditDate));
  } else {
    const priorRk = prior?.metrics?.rankings || {};
    if (prior?.send_status === 'sent' && priorRk.page1 != null) {
      priorSum = {
        buckets: priorRk.buckets || {},
        page1: priorRk.page1,
        tracked: priorRk.tracked,
        avg_position: priorRk.avg_position,
        money_avg_position: priorRk.money_avg_position
      };
    }
  }
  const priorReady = priorSum != null && priorSum.page1 != null;
  ...
  const priorRkSnap = prior?.send_status === 'sent' ? (prior?.metrics?.rankings || {}) : {};
  ...
  const bucketWow = (key) => weekSnapWow(
    cur.buckets[key],
    priorReady ? priorSum.buckets?.[key] : null
  );

  return {
    audit_date: auditDate,
    prior_audit_date: priorAuditDate,
    buckets: cur.buckets,
    page1: cur.page1,
    tracked: cur.tracked,
    avg_position: cur.avg_position,
    money_avg_position: cur.money_avg_position,
    ...
    buckets_pending: !priorReady,
    wow: {
      page1: weekSnapWow(cur.page1, priorReady ? priorSum.page1 : null),
      tracked: weekSnapWow(cur.tracked, priorReady ? priorSum.tracked : null),
      site_clicks: weekSnapWow(num(audit?.gsc_clicks), priorRkSnap.site_clicks ?? null),
      avg_position: weekSnapWow(cur.avg_position, priorReady ? priorSum.avg_position : null),
      money_avg_position: weekSnapWow(cur.money_avg_position, priorReady ? priorSum.money_avg_position : null),
      b1: bucketWow('b1'),
      b2_3: bucketWow('b2_3'),
      b4_10: bucketWow('b4_10'),
      b11_20: bucketWow('b11_20'),
      b21: bucketWow('b21')
    }
  };
}
...
async function sectionBacklinks(supabase, prior) {
  ...
  // Headline + DA-band Δ only vs a prior *sent* Monday snapshot
  const priorBlReady = prior?.send_status === 'sent'
    && (priorBl.referring_domains != null || priorBl.backlinks != null || priorBl.rank != null);
  ...
  const spam = num(cache?.backlinks_spam_score);
  ...
  const priorForLinks = priorBlReady ? priorBl : {};
  let linkSnap = { high_da_dofollow_keys: [], new_links: { status: 'from_next_monday', items: [] } };
  try {
    linkSnap = await loadHighDaDofollowSnapshot(supabase, priorForLinks);
  } catch (e) {
    linkSnap = {
  ...
  return {
    ...
    spam_score: spam,
    spam_label: (spam ?? 99) <= 30 ? 'low' : 'watch',
    ...
    wow: {
      referring_domains: weekSnapWow(blRd, priorBlReady ? priorBl.referring_domains : null),
      backlinks: weekSnapWow(blTotal, priorBlReady ? priorBl.backlinks : null),
      rank: weekSnapWow(num(cache?.rank), priorBlReady ? priorBl.rank : null),
      spam_score: weekSnapWow(spam, priorBlReady ? priorBl.spam_score : null),
      dofollow: weekSnapWow(blDf, priorBlReady ? priorBl.dofollow : null),
      nofollow: weekSnapWow(blNf, priorBlReady ? priorBl.nofollow : null)
    }
  };
}
...
function buildNarrative(metrics) {
  ...
  if (roll.amount != null && roll.amount < rt.survival) {
    bad.push(`${fmtGbp(Math.abs(rt.gap_vs_survival || 0))} under survival`);
    worries.push(`Revenue 28d ${roll.wow?.label || ''}`.trim());
  }
  ...
  const week_verdict = orgPct != null && orgPct > 0 && (roll.amount || 0) < (rt.survival || 0)
  ...
  const moneyWord = (roll.amount || 0) >= (rt.survival || DEFAULT_TIER_BANDS.survival) ? 'HEALTHY' : 'DECLINE';
  const underGap = Math.abs(rt.gap_vs_survival || 0);
  const underBit = (roll.amount || 0) < (rt.survival || DEFAULT_TIER_BANDS.survival)
    ? `, <b style="color:#b42318;">${fmtGbp(underGap)} under survival</b>`
    : `, <b style="color:#067647;">above survival</b>`;
  const trendBit = roll.wow?.label
    ? `. ${roll.wow.delta < 0 ? 'Down' : (roll.wow.delta > 0 ? 'Up' : 'Flat')} <b style="color:${roll.wow.delta < 0 ? '#b42318' : '#067647'};">${roll.wow.label}</b> vs prior 28d`
    : '';
  ...
  const moneyVerdict = {
    ...
    line: `${fmtGbp(roll.amount)} (28d)${underBit}${trendBit}${dropBit}.${riseBit}`
  };
...
/** Same Current Month Pulse tile as Revenue Truth (non-JLR worst-case projected). */
async function fetchCurrentMonthPulse() {
  const res = await fetch(`${appBase()}/api/aigeo/revenue-truth-summary?includeJlr=false`, {
    headers: { Accept: 'application/json' }
  });
  if (!res.ok) throw new Error(`revenue_truth_summary_${res.status}`);
  const json = await res.json();
  const pulse = json?.currentMonthPulse || json?.current_month_pulse || null;
  if (!pulse) return null;
  const defcon = pulse.defcon || {};
  return {
    month_label: pulse.month_label || null,
    days_remaining: pulse.days_remaining ?? null,
    days_elapsed: pulse.days_elapsed ?? null,
    booked_so_far: pulse.booked_nonjlr_so_far ?? null,
    projected_month_end: defcon.projected_month_end ?? pulse.projections?.worst_case_month_end ?? null,
    survival: defcon.survival_line ?? DEFAULT_TIER_BANDS.survival,
    miss_vs_survival: defcon.miss_vs_survival_gbp
      ?? (defcon.projected_month_end != null
        ? Math.round((DEFAULT_TIER_BANDS.survival - Number(defcon.projected_month_end)) * 100) / 100
        : null),
    pct_of_survival: defcon.pct_of_survival ?? null,
    defcon_level: defcon.level ?? null,
    defcon_status: defcon.status ?? null,
    source: 'revenue-truth-summary currentMonthPulse (same as dashboard tile)'
  };
}
...
export async function buildCeoWeeklyMetrics(supabase, opts = {}) {
  ...
  const revenue_by_line = await sectionRevenueByLine(supabase);
  ...
  try { current_month_pulse = await fetchCurrentMonthPulse(); } catch (e) { current_month_pulse = { error: e.message }; }
  ...
  const metrics = {
    ...
    definitions: {
      ...
      revenue_lines: 'booking_sheet_transactions non-JLR — rolling 28d vs prior 28d',
      rankings: 'keyword_rankings latest vs prior audit_date for bucket/avg Δ',
      acquisition: 'GET /api/aigeo/acquisition-channels',
      surfaces: 'computeSurfaceOutcomesRollup on money keywords',
      survival: `DEFAULT_TIER_BANDS.survival = ${DEFAULT_TIER_BANDS.survival} (compared to rolling 28d sales)`,
      ...
      click_movers: 'get-gsc-csv40d-click-movers (csv40d pair or latest dated gsc_page_metrics_28d runs)',
