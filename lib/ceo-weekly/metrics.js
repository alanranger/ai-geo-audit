...
async function sectionRankings(supabase, propertyUrl, prior) {
  ...
  const priorRkReady = prior?.send_status === 'sent' && priorRk && priorRk.page1 != null;
  ...
  const bucketWow = (key) => weekSnapWow(
    buckets[key],
    priorRkReady ? priorRk.buckets?.[key] : null
  );
  ...
  return {
    ...
    buckets_pending: !priorRkReady,
    wow: {
      page1: weekSnapWow(page1, priorRkReady ? priorRk.page1 : null),
      tracked: weekSnapWow(tracked, priorRkReady ? priorRk.tracked : null),
      site_clicks: weekSnapWow(num(audit?.gsc_clicks), priorRkReady ? priorRk.site_clicks : null),
      avg_position: weekSnapWow(avgPos, priorRkReady ? priorRk.avg_position : null),
      money_avg_position: weekSnapWow(moneyAvg, priorRkReady ? priorRk.money_avg_position : null),
      b1: bucketWow('b1'),
      b2_3: bucketWow('b2_3'),
