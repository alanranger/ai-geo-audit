/**
 * Monday 01:00 orchestrator — Dashboard Standard warm path (not Full/DFS-heavy).
 * Syncs + GSC & Backlink Audit + Ranking & AI, then gate for the CEO report.
 */
...
/**
 * Ordered Standard warm steps.
 * GSC audit + Ranking & AI use force=1 so Monday is not schedule-gated.
 */
const STEPS = [
  { key: 'squarespace_revenue', path: '/api/aigeo/squarespace-revenue-sync' },
  { key: 'stripe_revenue', path: '/api/aigeo/stripe-revenue-sync' },
  // POST: GET without x-vercel-cron returns 401 on these handlers
  ...
  // Dashboard Standard: GSC & Backlink Audit + Ranking & AI (server cron equivalents)
  {
    key: 'gsc_backlink_audit',
    path: `/api/cron/daily-gsc-backlink?force=1&propertyUrl=${encodeURIComponent(DEFAULT_PROPERTY)}`,
    method: 'GET'
  },
  {
    key: 'ranking_ai',
    path: `/api/cron/keyword-ranking-ai?force=1&propertyUrl=${encodeURIComponent(DEFAULT_PROPERTY)}`,
    method: 'GET'
  }
];
