...
export async function persistBookingSheetTruth(supabase, propertyUrl, parsed) {
  ...
  const catRows = parsed.monthlyPerCategory || [];
  const txnRows = dropTxnGeneratedFields(parsed.transactionRows || []);
  if (!catRows.length) {
    throw new Error(
      'Refusing import: no monthly category revenue parsed. '
      + 'Open the workbook in Excel, recalculate (Ctrl+Alt+F9), save, and upload again.'
    );
  }
  const gpRows = reconcileGpLabels(parsed.gpPerCategory || [], catRows, warnings);
  await clearBookingSheetForProperty(supabase, propertyUrl);
  await upsertChunks(supabase, 'booking_sheet_monthly_category', catRows);
  ...
  return {
    category_rows_written: catRows.length,
