...
function renderHubProductTable(payload, tier, windowMonths, includeJlr) {
  ...
  const foot = products.length ? `<tfoot><tr class="rt-grand-total">`
    ...
    + `<td>${fmtMoney(totalLife, 0)}</td><td>${fmtMoney(totalWin, 0)}</td>`
    + `<td></td><td></td>`
  ...
  const winTip = `Booked revenue in the selected ${windowMonths}-month diagnosis window (Booking Sheet)`;
  return `<div class="rt-hub-products-wrap"><div class="rt-sub">${filterNote}</div>`
    ...
    + `<th>${tip('Txns', 'Lifetime transaction count (Booking Sheet)')}</th>`
    + `<th>${tip('Lifetime £', 'Total booked revenue for this product, all time')}</th>`
    ...
    + `</tr></thead><tbody>${rows || '<tr><td colspan="14">No products mapped.</td></tr>'}</tbody>${foot}</table></div>`;
}
