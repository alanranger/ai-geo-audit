...
function renderHubProductTable(payload, tier, windowMonths, includeJlr) {
  ...
  const rows = products.map((p) => {
    ...
    const landingTag = p.landing_attributed
      ? ` <span class="rt-diag-pill" style="font-size:0.55rem;padding:0.08rem 0.4rem;margin-left:0.35rem;letter-spacing:0.03em;" title="Attributed to this hub by its Booking Sheet landing-page tag (Column J), not the product tag \u2014 e.g. combined Acuity bookings.">landing-tagged</span>`
      : '';
    const titleCell = (path ? slugLink(path, escapeHtml(p.product_title)) : escapeHtml(p.product_title)) + landingTag;
