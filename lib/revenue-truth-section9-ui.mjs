...
function renderRevenueBlock(t) {
  ...
  return `<div class="rt-diag-tier-rolegsc-col is-revenue">`
    + `<div class="rt-diag-tier-cell-label">Revenue trend (Booking Sheet, non-JLR)</div>`
...
function renderTierRoleGscPair(t, windowMonths) {
...
function renderHubProductTable(payload, tier, windowMonths, includeJlr) {
  ...
  const rows = (payload?.products || []).map((p) => {
    ...
    return `<tr class="${zero ? 'is-zero' : ''}">`
      ...
      + `<td>${tip('Txns', 'Transaction count (lifetime)')} ${p.lifetime_txn_count || 0}</td>`
      + `<td>${tip('Lifetime £', 'Lifetime booked revenue')}${fmtMoney(p[lifeKey] || 0, 0)}</td>`
      + `<td>${tip('Window £', 'Revenue in overlay window')}${fmtMoney(p[winKey] || 0, 0)}</td>`
      + `<td>${escapeHtml(p.lifetime_first_txn || '—')}</td>`
      + `<td>${escapeHtml(p.lifetime_last_txn || '—')}</td>`
      + `<td>${fmtN(g.impressions || 0)}</td>`
      + `<td>${fmtN(g.clicks || 0)}</td>`
      + `<td>${tip('CTR', 'Click-through rate')}${diagCtrPct(g.clicks, g.impressions)}</td>`
      + `<td>${tip('Pos', 'Average position')}${diagPosLabel(g.best_avg_position)}</td>`
  ...
  return `<div class="rt-hub-products-wrap"><div class="rt-sub">${filterNote}</div>`
    ...
    + `<th>12mo</th><th>Product</th><th>Season</th><th>Price</th><th>Txns</th><th>Lifetime £</th><th>Window £</th><th>First</th><th>Last</th><th>Imp</th><th>Clicks</th><th>CTR</th><th>Pos</th><th>Δ</th>`
    + `</tr></thead><tbody>${rows || '<tr><td colspan="14">No products mapped.</td></tr>'}</tbody></table></div>`;
}
...
function renderDiagCardHtml(d, tier, hubBreakdown, windowMonths, includeJlr) {
  ...
  return `<div class="rt-diag-card severity-${sev}" data-slug="${escapeHtml(d.page_slug)}">`
    ...
    + `<div class="rt-diag-meta-tile"><div class="rt-diag-meta-label">Window £</div><div class="rt-diag-meta-value">${fmtMoney(fw.revenue_gbp_nonjlr || 0, 0)}</div></div>`
    + `</div>`
...
export function renderTierRowHtml(t, pages, options = {}) {
  ...
  return `<div class="rt-diag-tier-row severity-${sev} ${expanded ? '' : 'is-collapsed'}" data-tier="${escapeHtml(t.tier_key)}">`
    ...
    + tierCardSignals(t)
    + `</div>`
    + `<div class="rt-diag-tier-revtrend"><div class="rt-diag-tier-cell-label">Revenue / Hub / Product</div>${renderTierRoleGscPair(t, windowMonths)}</div>`
    + `<div class="rt-diag-tier-pills"><div class="rt-diag-tier-cell-label" style="text-align:right;">Page states</div>${renderTierStateChips(t.page_state_counts, sev)}</div>`
    + `</div>`
