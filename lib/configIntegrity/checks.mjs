...
const LOCKED_CSV = path.join(ROOT, 'config/keyword-tracking-locations-and-class-LOCKED-v11.csv');
...
/** LOCKED may intentionally own a blog (content SERP) — not a commercial-page bug. */
function isIntentionalContentTarget(tp) {
  const p = String(tp || '').toLowerCase();
  return p.includes('/blog-on-photography/');
}
...
export function checkLockedTargetPages(lockedByKw, pagesMap) {
  ...
  for (const [, row] of lockedByKw) {
    ...
    // e.g. food photography → food-photography-tips blog (Alan accept ranked content page)
    if (isIntentionalContentTarget(tp)) continue;
