/**
 * Enrich check-2 / check-3 findings with meaning, at_stake, suggested_action, workstream.
 * Presentation only — no check logic changes.
 * Stream tags use Hub/Swap/Blog/Config (not "WS3") so they never collide with the retired recrawl panel.
 */
...
function tagWorkstream(keyword, preferred, assigned, mutualSwap) {
  ...
  if (mutualSwap.has(nk)) return 'Swap';
  if (HIRE_SWAP.has(preferred) && HIRE_SWAP.has(assigned)) return 'Swap';
  if (WORKSHOP_SWAP.has(preferred) || WORKSHOP_SWAP.has(assigned)) return 'Swap';
  if (preferred === COURSES_HUB) return 'Hub';
  if (preferred.includes('/blog-on-photography/')) return 'Blog';
  if (preferred === FREE_COURSE) return 'Blog';
...
function suggestedAction3({ keyword, preferred, assigned, pagesMap, mutualSwap, otherPage }) {
  ...
  if (mutualSwap.has(normKw(keyword))) {
    return `Crossed targets with ${otherPage || 'another money page'} — decision needed (Swap stream).`;
  }
  ...
  if (preferred === COURSES_HUB && assigned !== COURSES_HUB) {
    return 'Decision needed (accept hub + route, or strengthen specialist) — Hub stream.';
  }
...
function enrichCheck2(finding, ctx) {
  ...
  if (cannibal) {
    ...
    ws = lockedTarget === COURSES_HUB ? 'Hub' : 'Swap';
  } else if (notLocked) {
    keyword = notLocked[1];
