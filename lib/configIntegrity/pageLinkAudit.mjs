...
export function statusKeyFromLink(linkStatus, isDecision) {
  ...
  if (linkStatus === 'present') {
    return { key: 'awaiting', label: 'Link in place — awaiting recrawl', sort: 1 };
  }
  if (linkStatus === 'weak') {
    return { key: 'weak', label: 'Link present, weak anchor', sort: 2 };
  }
  if (linkStatus === 'absent') {
    return { key: 'fix', label: 'Fix needed', sort: 3 };
  }
  return { key: 'unchecked', label: 'Not page-checked', sort: 4 };
}
