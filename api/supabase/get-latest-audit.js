        ...
        if (keywordRows && keywordRows.length > 0) {
          ...
          const combinedRows = keywordRows.map(row => {
            ...
            return {
              ...
              serp_surface_stack: Array.isArray(row.serp_surface_stack)
                ? row.serp_surface_stack
                : (typeof row.serp_surface_stack === 'string'
                  ? (() => { try { const parsed = JSON.parse(row.serp_surface_stack); return Array.isArray(parsed) ? parsed : null; } catch { return null; } })()
                  : null),
