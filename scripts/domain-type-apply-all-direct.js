/**
 * Classify and apply all 252 domains directly via SQL
 */
...
// Generate SQL batches (50 domains per batch for safe execution)
const batchSize = 50;
const sqlBatches = [];
...
for (let i = 0; i < classifications.length; i += batchSize) {
  ...
  sqlBatches.push({ batchNum: Math.floor(i / batchSize) + 1, sql, count: batch.length });
}

console.log(`✅ Generated ${sqlBatches.length} SQL batches for ${classifications.length} domains\n`);

// Output SQL batches for manual application
sqlBatches.forEach((batch, idx) => {
  console.log(`\n--- Batch ${batch.batchNum} (${batch.count} domains) ---`);
