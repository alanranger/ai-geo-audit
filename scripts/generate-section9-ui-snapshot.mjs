...
// Static shareable snapshot of current §9 UI structure (May 2026).
...
function renderDiagCardHtml(d, tier, breakdownSample) {
  ...
  const fw = d.metrics?.full_window || {};
  const lifetime = d.metrics?.lifetime;
  let drill = '';
...
const tierRows = rollup.map((t) => {
  ...
  const expanded = EXPAND_TIERS.has(t.tier_key);
...
const html = `<!DOCTYPE html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>§9 Revenue Funnel Diagnosis — full UI snapshot</title><style>${css}</style></head><body><div class="wrap"><div class="banner"><strong>§9 UI snapshot</strong> — generated ${new Date().toISOString().slice(0, 19).replace('T', ' ')} UTC · live diagnosis API · share with Claude for UX guidance<br><span class="muted">Expanded tiers: Workshops Non Residential, Courses/Masterclasses, Workshops Residential · collapsed: all others · Sparklines shown as placeholders</span></div><div class="rt-diag-section"><div class="rt-diag-header"><h3>9. Revenue Funnel Diagnosis <span class="rt-diag-pill">TIER → PAGE → PRODUCT</span></h3><p class="rt-sub">Level 1 tier row (revenue + Hub|Product GSC) · expand → slug tables + page cards · mixed pages → Level 3 revenue breakdown. GSC overlay Jan 2025+ only.</p></div><div class="rt-diag-status">${escapeHtml(statusLine)}</div><div class="rt-diag-tier-list-header"><div>Tier</div><div>Revenue trend</div><div>GSC by page role</div><div style="text-align:right;">Page states</div></div><div class="rt-diag-tier-list">${tierRows}</div><div class="legend-box"><strong>Structure note (current build, not target hub→nested-products tree):</strong><ul><li>Tier row = revenue + aggregated Hub total + aggregated Product total</li><li>Expand = flat hub slug table + flat product slug table (not grouped under each hub)</li><li>Page cards = diagnostic hub URLs only; each shows this-page GSC vs tier product total</li><li>Level 3 on landscape-photography-workshops sample = revenue by product, no GSC</li><li><em>Not yet built:</em> hub card → products mapped to that hub with per-product GSC nested underneath</li></ul></div></div></div></body></html>`;
